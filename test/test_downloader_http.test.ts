...
describe("HTTP downloader", () => {
  ...
  async function download(
    ...
    const downloader = new HttpFD(fakeDownloader(), {
      ...params,
      quiet: true,
      noprogress: true,
    });
    expect(
      await downloader.realDownload(filename, {
...
function fakeDownloader() {
  return {
    ...
    async urlopen(url: string | URL | Request) {
      const request =
        url instanceof Request ? url : new Request(url.toString());
...
function serveTestRequest(request: Request): Response {
  ...
  const supportsRange =
    path !== "/no-range" && path !== "/no-range-no-content-length";
  const sendsContentLength =
    path !== "/no-content-length" && path !== "/no-range-no-content-length";
  ...
  return new Response(Buffer.alloc(size, "#"), {
