...
import {
  ...
  parseFragment,
  parseTs,
} from "../yt_dlp/webvtt.ts";

describe("WebVTT parser", () => {
  test("timestamp parse/format", () => {
    const match = /(?:([0-9]{1,}):)?([0-9]{2}):([0-9]{2})\.([0-9]{3})?/.exec(
      "01:02:03.456",
    );
    if (!match) {
      throw new Error("expected timestamp regex to match");
    }
  ...
  test("CueBlock JSON roundtrip and hinges", () => {
    ...
    const second = CueBlock.fromJson({
      id: undefined,
      start: 90_000,
      end: 180_000,
      settings: "line:0",
