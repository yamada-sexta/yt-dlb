...
import type {
  DownloadInfo,
  DownloaderHost,
} from "../yt_dlp/downloader/common.ts";
import {
  Aria2cFD,
  AxelFD,
  CurlFD,
  ...
  outputFormat,
  outputFormatForInfo,
} from "../yt_dlp/downloader/external.ts";
...
function fakeYdl(
  cookieHeader?: string,
): DownloaderHost & { cookies?: unknown } {
  return {
    params: {},
    cookies: cookieHeader
      ? {
          ...
          getCookiesForUrl: () => [
            { name: "test", value: "ytdlp", path: "/", domain: ".example.com" },
          ],
        }
      : {
...
describe("external downloader command builders", () => {
  test("HttpieFD command and Cookie header", async () => {
    expect(
      await new TestHttpieFD(fakeYdl()).makeCmdPublic("test", TEST_INFO),
    ).toEqual([
      "http",
      "--download",
      "--output",
    ...
    expect(
      await new TestHttpieFD(fakeYdl(TEST_COOKIE)).makeCmdPublic(
        "test",
        TEST_INFO,
      ),
    ).toEqual([
  ...
  test("AxelFD command and Cookie header", async () => {
    expect(
      await new TestAxelFD(fakeYdl()).makeCmdPublic("test", TEST_INFO),
    ).toEqual(["axel", "-o", "test", "--", "http://www.example.com/"]);
    expect(
      await new TestAxelFD(fakeYdl(TEST_COOKIE)).makeCmdPublic(
        "test",
        TEST_INFO,
      ),
    ).toEqual([
  ...
  test("WgetFD command adds cookie file when cookies exist", async () => {
    expect(
      await new TestWgetFD(fakeYdl()).makeCmdPublic("test", TEST_INFO),
    ).not.toContain("--load-cookies");
    expect(
      await new TestWgetFD(fakeYdl(TEST_COOKIE)).makeCmdPublic(
        "test",
        TEST_INFO,
      ),
    ).toContain("--load-cookies");
    expect(
      await new TestWgetFD(fakeYdl(TEST_COOKIE)).makeCmdPublic(
        "test",
        TEST_INFO,
      ),
    ).toContain(TEST_COOKIE_FILE);
  });

  test("CurlFD command adds cookie header when cookies exist", async () => {
    expect(
      await new TestCurlFD(fakeYdl()).makeCmdPublic("test", TEST_INFO),
    ).not.toContain("--cookie");
    const cmd = await new TestCurlFD(fakeYdl(TEST_COOKIE)).makeCmdPublic(
      "test",
      TEST_INFO,
    );
  ...
  test("Aria2cFD command adds cookie file when cookies exist", async () => {
    expect(
      await new TestAria2cFD(fakeYdl()).makeCmdPublic("test", TEST_INFO),
    ).not.toContain(`--load-cookies=${TEST_COOKIE_FILE}`);
    expect(
      await new TestAria2cFD(fakeYdl(TEST_COOKIE)).makeCmdPublic(
        "test",
        TEST_INFO,
      ),
    ).toContain(`--load-cookies=${TEST_COOKIE_FILE}`);
  });
  ...
  test("ffmpeg output format helpers", () => {
    ...
    expect(
      outputFormatForInfo("test.mp4", { url: TEST_INFO.url, protocol: "rtmp" }),
    ).toBe("flv");
  });

  test("FFmpegFD full command assembly", () => {
    expect(
      new TestFFmpegFD(fakeYdl()).makeCmdPublic("test", {
        ...TEST_INFO,
        ext: "mp4",
      }),
    ).toEqual([
    ...
    expect(
      new TestFFmpegFD(fakeYdl(TEST_COOKIE)).makeCmdPublic("test", {
        ...TEST_INFO,
        ext: "mp4",
      }),
    ).toEqual([
