...
describe("networking utility helpers", () => {
  ...
  test("HTTPHeaderDict is case-insensitive but preserves sensitive casing", () => {
    ...
    expect(headers.sensitive()).toEqual({ "Content-type": "application/json", "X-Test": "ok" });
  });

  test("cleanProxies and selectProxy", () => {
    const headers = new HTTPHeaderDict({ "Ytdl-Request-Proxy": "socks5://example.com" });
    ...
    expect(selectProxy("https://example.com", proxies)).toBe("socks5://example.com");
  });
...
describe("networking exceptions", () => {
  test("HTTPError exposes status and response", () => {
    const response = new Response("test", { status: 403, statusText: "Forbidden" });
