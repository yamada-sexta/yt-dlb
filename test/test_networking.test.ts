...
describe("networking common request objects", () => {
  ...
  test("RequestHandler validates supported URL schemes and proxies", () => {
    ...
    expect(() => handler.validate(new Request("ftp://example.com"))).toThrow(UnsupportedRequest);
    expect(() => handler.validate(new Request("http://example.com", { proxies: { all: "socks5://example.com" } }))).toThrow(UnsupportedRequest);
    expect(() => handler.validate(new Request("http://example.com", { extensions: { unknown: true } }))).toThrow(UnsupportedRequest);
  });

  test("RequestDirector reports no handlers", async () => {
    await expect(new RequestDirector().send(new Request("https://example.com"))).rejects.toThrow("No request handlers configured");
  });
  ...
  test("RequestDirector falls through unsupported handlers", async () => {
    ...
    await expect(director.send(new Request("https://example.com"))).rejects.toThrow(NoSupportingHandlers);
  });
});
...
describe("impersonation targets", () => {
  ...
  test("request handler rejects unsupported impersonation", () => {
    class ChromeRH extends ImpersonateRequestHandler {
      static override readonly SUPPORTED_IMPERSONATE_TARGETS = [new ImpersonateTarget("chrome")];
      protected override async doSend(_request: Request): Promise<Response> {
        return new Response();
      }
    }
    const handler = new ChromeRH();
    expect(() => handler.validate(new Request("https://example.com", {
