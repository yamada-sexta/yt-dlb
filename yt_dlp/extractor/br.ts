...
export class BRIE extends InfoExtractor {
  ...
  static override readonly _VALID_URL = String.raw`(?<base_url>https?://(?:www\.)?br(?:-klassik)?\.de)/(?:[a-z0-9\-_]+/)+(?<id>[a-z0-9\-_]+)\.html`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    if (typeof xmlUrl !== "string") {
      throw new ExtractorError("Unable to extract XMLURL", { videoId: displayId });
    }
    const xml = await this.downloadXml(`${baseUrl}${xmlUrl}`, displayId);
    if (xml === false) {
      throw new ExtractorError("Unable to download XML", { videoId: displayId });
    }

    const medias: ExtractorInfo[] = [];
    for (const xmlMedia of [...xml.children.filter((child) => child.tag === "video"), ...xml.children.filter((child) => child.tag === "audio")]) {
      const mediaId = xmlMedia.attrib.externalId;
      const assets = xpathElement(xmlMedia, "assets");
      medias.push({
        id: mediaId,
        title: xpathText(xmlMedia, "title", "title", { fatal: true }) ?? undefined,
        duration: parseDuration(xpathText(xmlMedia, "duration")),
        formats: this.extractFormats(assets, mediaId ?? displayId),
        thumbnails: this.extractThumbnails(xpathElement(xmlMedia, "teaserImage/variants"), baseUrl),
    ...
    if (medias.length > 1) {
      this.reportWarning("found multiple medias; please report this with the video URL to http://yt-dl.org/bug");
    }
    if (!medias.length) {
      throw new ExtractorError("No media entries found", { videoId: displayId });
    }
    return medias[0]!;
  }

  private extractFormats(assets: XmlElement | null, mediaId: string): Array<Record<string, unknown>> {
    const formats: Array<Record<string, unknown>> = [];
    for (const asset of assets?.children.filter((child) => child.tag === "asset") ?? []) {
      ...
      if (assetType.startsWith("HDS")) {
        formats.push(...this.extractF4mFormats(`${formatUrl}?hdcore=3.2.0`, mediaId, { f4mId: "hds", fatal: false }));
      } else if (assetType.startsWith("HLS")) {
        formats.push(...this.extractM3u8Formats(formatUrl, mediaId, "mp4", { entryProtocol: "m3u8_native", m3u8Id: "hds" }));
      } else {
        const formatInfo = {
  ...
  private extractThumbnails(variants: XmlElement | null, baseUrl: string): Array<Record<string, unknown>> {
    const thumbnails = (variants?.children.filter((child) => child.tag === "variant") ?? [])
      .flatMap((variant) => {
...
function uploadDateInfo(value: string | null): Record<string, string> {
  ...
  return parts?.length === 3 ? { upload_date: parts.toReversed().join("") } : {};
}
