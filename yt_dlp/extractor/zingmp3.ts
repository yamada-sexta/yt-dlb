...
abstract class ZingMp3BaseIE extends InfoExtractor {
  static readonly VALID_URL_TMPL =
    String.raw`https?://(?:mp3\.zing|zingmp3)\.vn/(?<type>(?:%s))/[^/?#]+/(?<id>\w+)(?:\.html|\?)`;
  ...
  protected apiUrl(
    ...
    if (!apiSlug) {
      throw new ExtractorError(`Unsupported Zing MP3 API type ${urlType}`, {
        expected: true,
      });
    }
    const allParams = { ...params, ctime: "1" };
    const signatureInput = Object.entries(allParams)
      .sort(([left], [right]) => left.localeCompare(right))
      .map(([key, value]) => `${key}=${value}`)
      .join("");
    const sha256 = createHash("sha256").update(signatureInput).digest("hex");
    const sig = createHmac("sha512", "acOrvUS15XRW2o9JksiK1KgQ6Vbds8ZW")
      .update(`${apiSlug}${sha256}`)
      .digest("hex");
  ...
  protected async callApi<T = Record<string, unknown>>(
    urlType: string,
    params: Record<string, string | number>,
    displayId: string | null = null,
    options: { fatal?: boolean; query?: Record<string, string> } = {},
    ...
    const resp = await this.downloadJson<{ data?: T }>(
      url,
      displayId ?? String(params.id ?? ""),
      {
  ...
  protected override async realInitialize(): Promise<void> {
    await this.requestWebpage(this.apiUrl("bai-hat", { id: "" }), "zingmp3", {
  ...
  protected async pagedList(
    ...
    for (let page = 1; ; page += 1) {
      ...
      if (
        !data.hasMore ||
        (intOrNone(data.total) !== null && count > intOrNone(data.total)!)
      ) {
        break;
      }
    }
...
export class ZingMp3IE extends ZingMp3BaseIE {
  static override readonly _VALID_URL = ZingMp3BaseIE.VALID_URL_TMPL.replace(
    "%s",
    "bai-hat|video-clip|embed|eps",
  );
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const item = await this.callApi<Record<string, unknown>>(urlType, {
      id: songId,
    });
    ...
    if (urlType === "video-clip") {
      ...
      const videoInfo = await this.downloadJson<{ source?: unknown }>(
        ...
        itemId,
        {
          query: { requestdata: JSON.stringify({ id: itemId }) },
          note: "Downloading mp4 JSON metadata",
        },
      );
      source.mp4 = videoInfo ? videoInfo.source : undefined;
    } else if (urlType === "eps") {
      source = await this.callApi<Record<string, unknown>>(
        "episode-streaming",
        { id: itemId },
      );
    } else {
      source = await this.callApi<Record<string, unknown>>("song-streaming", {
        id: itemId,
      });
    }
    ...
    for (const [key, value] of Object.entries(source ?? {})) {
      ...
      for (const [res, videoUrl] of Object.entries(getRecord(value))) {
        ...
        if (key === "hls") {
          formats.push(
            ...this.extractM3u8Formats(String(videoUrl), itemId, "mp4", {
              m3u8Id: key,
            }),
          );
        } else {
          formats.push({
            format_id: `mp4-${res}`,
            url: String(videoUrl),
            height: intOrNone(res) ?? undefined,
          });
        }
      }
    }
    if (!formats.length) {
      if (
        item.msg === "Sorry, this content is not available in your country."
      ) {
        this.raiseGeoRestricted(undefined, {
          countries: (this.constructor as typeof ZingMp3BaseIE)._GEO_COUNTRIES,
          metadataAvailable: true,
        });
      }
      this.raiseNoFormats("The song is only for VIP accounts.", {
        expected: true,
        videoId: itemId,
      });
    }
    const lyric =
      stringValue(item.lyric) ??
      stringValue(
        (
    ...
    return {
      ...
      artist:
        stringValue(item.artistsNames) ??
        stringValue(item.artists_names) ??
        stringValue(getPath(item, ["artists", 0, "name"])),
      album:
        stringValue(getPath(item, ["album", "name"])) ??
        stringValue(getPath(item, ["album", "title"])) ??
        stringValue(getPath(item, ["genres", 0, "name"])),
      album_artist:
        stringValue(getPath(item, ["album", "artistsNames"])) ??
        stringValue(getPath(item, ["album", "artists_names"])) ??
        stringValue(getPath(item, ["artists", 0, "name"])),
      formats,
...
export class ZingMp3AlbumIE extends ZingMp3BaseIE {
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const songId = match?.groups?.id ?? "";
    const urlType = match?.groups?.type ?? "";
    const data = await this.callApi<Record<string, unknown>>(urlType, {
      id: songId,
    });
...
export class ZingMp3ChartHomeIE extends ZingMp3BaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://(?:mp3\.zing|zingmp3)\.vn/(?<id>(?:zing-chart|moi-phat-hanh|top100|podcast-discover))/?(?:[#?]|$)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const items =
      urlType === "top100"
        ...
        : urlType === "zing-chart"
          ? asArray(getPath(data, ["RTChart", "items"]))
          : asArray(data.items);
...
export class ZingMp3WeekChartIE extends ZingMp3BaseIE {
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    return this.playlistResult(
      ...
      songId,
      `zing-chart-${stringValue(data.country) ?? ""}`,
    );
  }
}
...
export class ZingMp3ChartMusicVideoIE extends ZingMp3BaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://(?:mp3\.zing|zingmp3)\.vn/(?<type>the-loai-video)/(?<regions>[^/]+)/(?<id>[^.]+)`;
  ...
  protected override async fetchPage(
    ...
    return await this.callApi<Record<string, unknown>>(urlType, {
      id: songId,
      type: "genre",
      page,
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const urlType = match?.groups?.type ?? "";
    return this.playlistResult(
      await this.pagedList(songId, urlType),
      songId,
      `${urlType}_${regions}`,
    );
  }
...
export class ZingMp3UserIE extends ZingMp3BaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://(?:mp3\.zing|zingmp3)\.vn/(?<user>[^/]+)/(?<type>bai-hat|single|album|video|song)/?(?:[?#]|$)`;
  ...
  protected override async fetchPage(
    ...
    return await this.callApi<Record<string, unknown>>(
      urlType === "bai-hat" ? "user-list-song" : "user-list-video",
      {
        id: userId,
        type: "artist",
        page,
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const userInfo = await this.callApi<Record<string, unknown>>(
      "info-artist",
      {},
      alias,
      { query: { alias } },
    ...
    if (
      alias === "new-release" &&
      (urlType === "song" || urlType === "album")
    ) {
      const id = `${alias}-${urlType}`;
      return this.playlistResult(
        this.parseItems(
          await this.callApi("new-release", { type: urlType }, id),
        ),
        id,
      );
    }
    const entries =
      urlType === "bai-hat" || urlType === "video"
        ? await this.pagedList(stringValue(userInfo.id) ?? "", urlType)
        : this.parseItems(
            asArray(userInfo.sections)
              .filter(
                (section) =>
                  getRecord(section).sectionId ===
                  (urlType === "album" ? "aAlbum" : "aSingle"),
              )
              .flatMap((section) => asArray(getRecord(section).items)),
          );
    return this.playlistResult(
      entries,
      stringValue(userInfo.id) ?? null,
      joinNonempty(userInfo.name, urlType, { delim: " - " }),
      stringValue(userInfo.biography) ?? null,
    );
  }
}
...
export class ZingMp3HubIE extends ZingMp3BaseIE {
  ...
  static override readonly _VALID_URL =
    String.raw`https?://(?:mp3\.zing|zingmp3)\.vn/(?<type>hub)/[^/?#]+/(?<id>[^./?#]+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const songId = match?.groups?.id ?? "";
    const urlType = match?.groups?.type ?? "";
    const hub = await this.callApi<Record<string, unknown>>(urlType, {
      id: songId,
    });
    const entries = this.parseItems(
      asArray(hub.sections)
        .filter((section) => getRecord(section).sectionId === "hub")
        .flatMap((section) => asArray(getRecord(section).items)),
    );
    return this.playlistResult(
      entries,
      songId,
      stringValue(hub.title) ?? null,
      stringValue(hub.description) ?? null,
    );
  }
}
...
export class ZingMp3LiveRadioIE extends ZingMp3BaseIE {
  ...
  static override readonly _VALID_URL =
    String.raw`https?://(?:mp3\.zing|zingmp3)\.vn/(?<type>liveradio)/(?<id>\w+)(?:\.html|\?)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const info = await this.callApi<Record<string, unknown>>(urlType, {
      id: liveRadioId,
    });
    const manifestUrl = stringValue(info.streaming);
    if (!manifestUrl) {
      throw new ExtractorError("This radio is offline.", {
        expected: true,
        videoId: liveRadioId,
      });
    }
    ...
    return {
      ...
      thumbnail:
        urlOrNone(info.thumbnail) ??
        urlOrNone(info.thumbnailM) ??
        urlOrNone(info.thumbnailV) ??
        urlOrNone(info.thumbnailH) ??
        undefined,
...
export class ZingMp3PodcastEpisodeIE extends ZingMp3BaseIE {
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const entries = await this.pagedList(
      podcastId,
      urlType === "pgr" ? "pgr-list" : "cgr-list",
    );
    return this.playlistResult(
      entries,
      podcastId,
      stringValue(info.title) ?? null,
      stringValue(info.description) ?? null,
    );
  }
}
...
export class ZingMp3PodcastIE extends ZingMp3BaseIE {
  ...
  static override readonly _VALID_URL =
    String.raw`https?://(?:mp3\.zing|zingmp3)\.vn/(?<id>(?:cgr|top-podcast|podcast-new))/?(?:[#?]|$)`;
