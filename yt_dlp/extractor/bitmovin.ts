...
export class BitmovinIE extends InfoExtractor {
  static override readonly _VALID_URL =
    String.raw`https?://streams\.bitmovin\.com/(?<id>\w+)`;

  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    const videoId = this.matchId(url);
    const rawConfig = await this.downloadJson<unknown>(
      `https://streams.bitmovin.com/${videoId}/config`,
      videoId,
    );
    if (rawConfig === false) {
      throw new ExtractorError("Unable to download Bitmovin config", {
        videoId,
      });
    }
