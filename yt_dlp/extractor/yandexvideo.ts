...
import { bugReportsMessage, determineExt, intOrNone, parseQs, qualities, updateUrlQuery, urlOrNone } from "../utils/index.ts";
...
function nested(value: unknown, path: readonly (string | number)[]): unknown {
  ...
  for (const key of path) {
    current = typeof key === "number" ? (Array.isArray(current) ? current[key] : undefined) : record(current)[key];
  }
  return current;
}
...
export class YandexVideoIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://(?:yandex\.ru(?:/(?:portal/(?:video|efir)|efir))?/?\?.*?stream_id=|frontend\.vh\.yandex\.ru/player/)(?<id>(?:[\da-f]{32}|[\w-]{12}))`;

  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    const videoId = this.matchId(url);
    const graphql = await this.downloadJson<Record<string, unknown>>("https://frontend.vh.yandex.ru/graphql", videoId, {
      data: `{
  player(content_id: "${videoId}") {
    ...
    if (!Object.keys(player).length || player.error) {
      const response = await this.downloadJson<{ content?: Record<string, unknown> }>(`https://frontend.vh.yandex.ru/v23/player/${videoId}.json`, videoId, {
    ...
    const title = stringValue(player.title) ?? stringValue(player.computed_title) ?? videoId;
    ...
    for (const stream of streams) {
      ...
      if (ext === "m3u8") {
        formats.push(...this.extractM3u8Formats(contentUrl, videoId, "mp4", { entryProtocol: "m3u8_native", m3u8Id: "hls" }));
      } else if (ext === "mpd") {
        formats.push(...this.extractMpdFormats(contentUrl, videoId, { mpdId: "dash", fatal: false }));
      } else {
        formats.push({ url: contentUrl });
      }
    }

    const timestamp = intOrNone(player.release_date) ?? intOrNone(player.release_date_ut) ?? intOrNone(player.start_time);
...
export class YandexVideoPreviewIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://(?:www\.)?yandex\.\w{2,3}(?:\.(?:am|ge|il|tr))?/video/preview(?:/?\?.*?filmId=|/)(?<id>\d+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const data = typeof dataRaw === "string"
      ? this.parseJson<Record<string, unknown>>(dataRaw, videoId, { transform_source: lowercaseEscape }) ?? {}
      : {};
...
abstract class ZenYandexBaseIE extends InfoExtractor {
  protected async fetchSsrData(url: string, videoId: string): Promise<[string, Record<string, unknown>]> {
    let webpage = await this.downloadWebpage(url, videoId);
    if (webpage === false) {
      throw new Error("Unable to download Dzen webpage");
    }
    const redirect = stringValue(record(this.searchJson<Record<string, unknown>>(String.raw`(?:var|let|const)\s+it\s*=`, webpage, "redirect", videoId, { defaultValue: {} })).retpath);
    if (redirect) {
      videoId = this.matchId(redirect);
      webpage = await this.downloadWebpage(redirect, videoId, { note: "Redirecting" });
      if (webpage === false) {
        throw new Error("Unable to download redirected Dzen webpage");
      }
    }
    const metadata = this.searchJson<Record<string, unknown>>(
...
export class ZenYandexIE extends ZenYandexBaseIE {
  static override readonly _VALID_URL = String.raw`https?://(?:zen\.yandex|dzen)\.ru(?:/video)?/(?:media|watch)/(?:(?:id/[^/]+/|[^/]+/)(?:[a-z0-9-]+)-)?(?<id>[a-z0-9-]+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const streamUrls = new Set<string>([
      ...
      ...collectStrings(nested(videoData, ["video", "oneVideoStreams"]), "url"),
    ].flatMap((item) => {
    ...
    for (const streamUrl of streamUrls) {
      ...
      if (ext === "mpd" || contentType === "6") {
        const [fmts, subs] = await this.extractMpdFormatsAndSubtitles(streamUrl, videoId, { mpdId: "dash", fatal: false });
        formats.push(...fmts);
        subtitles = this.mergeSubtitles(subs, subtitles);
      } else if (ext === "m3u8" || contentType === "8") {
        const [fmts, subs] = await this.extractM3u8FormatsAndSubtitles(streamUrl, videoId, "mp4", { m3u8Id: "hls", fatal: false });
        formats.push(...fmts);
        subtitles = this.mergeSubtitles(subs, subtitles);
      } else if (contentType === "0") {
        const formatType = parseQs(streamUrl).type?.[0];
        formats.push({ url: streamUrl, format_id: formatType, ext: "mp4", quality: quality(formatType) });
      } else {
        this.reportWarning(`Unsupported stream URL: ${streamUrl}${bugReportsMessage()}`);
      }
    }
    ...
    return {
      ...
      duration: intOrNone(nested(videoData, ["video", "duration"])) ?? undefined,
      view_count: intOrNone(nested(videoData, ["video", "views"])) ?? undefined,
      timestamp: intOrNone(videoData.publicationDate) ?? undefined,
      tags: Array.isArray(videoData.tags) ? videoData.tags.filter((item): item is string => typeof item === "string") : undefined,
      uploader: stringValue(nested(videoData, ["source", "title"])) ?? undefined,
    };
  }
}

export class ZenYandexChannelIE extends ZenYandexBaseIE {
  static override readonly _VALID_URL = String.raw`https?://(?:zen\.yandex|dzen)\.ru/(?!media|video)(?:id/)?(?<id>[a-z0-9-_]+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const items = [...(Array.isArray(feedData.items) ? feedData.items : []), ...collectStrings(feedData, "link").map((link) => ({ link }))];
    for (const item of items) {
      const itemRecord = record(item);
      const link = urlOrNone(itemRecord.link);
      if (link) {
        entries.push(this.urlResult(link, ZenYandexIE, stringValue(itemRecord.id), stringValue(itemRecord.title)));
      }
    }
    return this.playlistResult(entries, channelId, stringValue(nested(channelData, ["channel", "source", "title"])), stringValue(nested(channelData, ["channel", "source", "description"])));
  }
}

function lowercaseEscape(source: string): string {
  return source.replaceAll(/\\u([0-9A-Fa-f]{4})/g, (_match, hex: string) => `\\u${hex.toLowerCase()}`);
}
