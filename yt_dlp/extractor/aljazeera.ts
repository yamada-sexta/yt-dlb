...
export class AlJazeeraIE extends InfoExtractor {
  static override readonly _VALID_URL =
    String.raw`https?://(?<base>\w+\.aljazeera\.\w+)/(?<type>programs?/[^/]+|(?:feature|video|new)s)?/\d{4}/\d{1,2}/\d{1,2}/(?<id>[^/?&#]+)`;
  static readonly BRIGHTCOVE_URL_RE =
    /https?:\/\/players\.brightcove\.net\/(?<account>\d+)\/(?<playerId>[a-zA-Z0-9]+)_(?<embed>[^/]+)\/index\.html\?videoId=(?<id>\d+)/;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const apiData = await this.downloadJson<Record<string, unknown>>(
      `https://${base}/graphql`,
      displayId,
      {
        query: {
    ...
    const video =
      tryGet(
        apiData,
        (value) =>
          (value as { data?: { article?: { video?: AlJazeeraVideo } } }).data
            ?.article?.video,
      ) ?? {};
    ...
    if (!videoId) {
      ...
      if (!videoId) {
        return this.urlResult(url, "Generic", null, null, {
