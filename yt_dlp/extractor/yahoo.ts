...
import { ExtractorError, cleanHtml, intOrNone, joinNonempty, mimetype2ext, parseIso8601, tryGet, urlOrNone } from "../utils/index.ts";
import { InfoExtractor, SearchInfoExtractor, type ExtractorInfo } from "./common.ts";
...
export class YahooIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`(?<url>https?://(?:(?<country>[a-zA-Z]{2}(?:-[a-zA-Z]{2})?|malaysia)\.)?(?:[\da-zA-Z_-]+\.)?yahoo\.com/(?:[^/]+/)*(?<id>[^?&#]*-[0-9]+(?:-[a-z]+)?)\.html)`;

  static override get IE_NAME(): string {
    return "yahoo";
  }

  private async extractYahooVideo(videoId: string, country: string): Promise<ExtractorInfo> {
    const initial = await this.downloadJson<JsonRecord>(`https://video-api.yql.yahoo.com/v1/video/sapi/streams/${videoId}`, videoId, { note: "Downloading video JSON metadata" }) as JsonRecord | false;
    const initialMediaObj = records(record(record(record(initial ? initial.query : null).results).mediaObj))[0] ?? {};
    ...
    for (const fmt of fmts) {
      const mediaResponse = await this.downloadJson<JsonRecord>(`https://video-api.yql.yahoo.com/v1/video/sapi/streams/${videoId}`, videoId, {
        note: `Downloading ${fmt} JSON metadata`,
        query: { format: fmt, region: region.toUpperCase() },
      }) as JsonRecord | false;
      const mediaObj = records(record(record(record(mediaResponse ? mediaResponse.query : null).results).mediaObj))[0] ?? {};
      ...
      for (const stream of records(mediaObj.streams)) {
        ...
        if (stream.format === "m3u8") {
          formats.push(...this.extractM3u8Formats(streamUrl, videoId, "mp4", { m3u8Id: "hls" }));
          continue;
        }
      ...
      for (const cc of records(mediaObj.closedcaptions)) {
        ...
        subtitles[lang] ??= [];
        subtitles[lang].push({ url: ccUrl, ext: mimetype2ext(stringValue(cc.content_type)) });
      }
    }
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    if (!pageUrl || !displayId) {
      throw new ExtractorError("Unable to extract Yahoo id", { expected: true });
    }
    country = country.split("-")[0] ?? "us";
    const article = await this.downloadJson<{ items?: JsonRecord[] }>(`https://${country}.yahoo.com/caas/content/article`, displayId, {
    ...
    if (item.type !== "video") {
      ...
      if (cover.type === "yvideo" && typeof cover.url === "string") {
        entries.push(this.urlResult(cover.url, "Yahoo", stringValue(cover.uuid)));
      }
      ...
      if (item.type === "storywithleadvideo") {
        const iframeUrl = stringValue(tryGet(item, (value) => record(record(record(value).meta).player).url));
        if (iframeUrl) {
          entries.push(this.urlResult(iframeUrl));
        } else {
          this.reportWarning("Yahoo did not provide an iframe url for this storywithleadvideo");
        }
      }
      return this.playlistResult(entries, stringValue(item.uuid), stringValue(item.title), stringValue(item.summary));
    }
    const uuid = stringValue(item.uuid);
    if (!uuid) {
      throw new ExtractorError("Unable to extract Yahoo video uuid", { videoId: displayId });
    }
    return {
...
export class YahooSearchIE extends SearchInfoExtractor {
  ...
  static override readonly _VALID_URL = String.raw`yvsearch(?<n>\d*)?:(?<id>.+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    for (let pageNum = 0; pageNum < 1; pageNum += 1) {
      ...
      const info = await this.downloadJson<JsonRecord>(resultUrl, query, { note: `Downloading results page ${pageNum + 1}` }) as JsonRecord | false;
...
export class YahooJapanNewsIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://news\.yahoo\.co\.jp/(?:articles|feature)/(?<id>[a-zA-Z0-9]+)`;
  ...
  private extractFormats(jsonData: JsonRecord, contentId: string): Array<Record<string, unknown>> {
    const formats: Array<Record<string, unknown>> = [];
    for (const video of records(jsonData).length ? collectRecords(jsonData, "VideoUrl") : []) {
      ...
      if (delivery === "hls") {
        formats.push(...this.extractM3u8Formats(url, contentId, "mp4", { entryProtocol: "m3u8_native", m3u8Id: "hls" }));
      } else {
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const state = this.searchJson<JsonRecord>(String.raw`__PRELOADED_STATE__\s*=`, webpage, "preloaded state", videoId) ?? {};
    const contentId = findFirstNumber(state, "vid");
    if (contentId === null) {
      throw new ExtractorError("This article does not contain a video", { expected: true, videoId });
    }
    const host = "news.yahoo.co.jp";
    const spaceId = findFirstString(state, "spaceId");
    const jsonData = await this.downloadJson<JsonRecord>(`https://feapi-yvpub.yahooapis.jp/v1/content/${contentId}`, videoId, {
      query: {
    ...
    return {
      id: videoId,
      title: findFirstString(state, "headline") ?? findFirstString(state, "title") ?? this.htmlSearchMeta(["og:title", "twitter:title"], webpage) ?? this.htmlExtractTitle(webpage) ?? videoId,
      description: findFirstString(state, "description") ?? this.htmlSearchMeta(["og:description", "description", "twitter:description"], webpage) ?? undefined,
      thumbnail: findFirstString(state, "ogpImage") ?? this.ogSearchThumbnail(webpage) ?? this.htmlSearchMeta("twitter:image", webpage) ?? undefined,
...
function findFirstString(value: unknown, key: string): string | null {
  ...
  for (const child of Object.values(itemRecord)) {
    const found = Array.isArray(child)
      ? child.map((item) => findFirstString(item, key)).find((item): item is string => typeof item === "string")
...
function findFirstNumber(value: unknown, key: string): number | null {
  ...
  for (const child of Object.values(itemRecord)) {
    const found = Array.isArray(child)
      ? child.map((item) => findFirstNumber(item, key)).find((item): item is number => typeof item === "number")
