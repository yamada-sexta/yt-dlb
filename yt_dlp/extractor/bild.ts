...
const BildSourceSchema = z.object({
  src: z.string(),
  type: z.string().optional(),
}).passthrough();

const BildDataSchema = z.object({
  title: z.string(),
...
export class BildIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://(?:www\.)?bild\.de/(?:[^/]+/)+(?<display_id>[^/]+)-(?<id>\d+)(?:,auto=true)?\.bild\.html`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    for (const src of videoData.clipList?.[0]?.srces ?? []) {
      if (src.type === "application/x-mpegURL") {
        formats.push(...this.extractM3u8Formats(src.src, videoId, "mp4", { m3u8Id: "hls" }));
      } else if (src.type === "video/mp4") {
        formats.push({ url: src.src, format_id: "http-mp4" });
      } else {
        this.reportWarning(`Skipping unsupported format type: "${src.type}"`, videoId);
      }
    }
    ...
    return {
      ...
      description: unescapeHTML(videoData.description ?? undefined) ?? undefined,
      formats,
