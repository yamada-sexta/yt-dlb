...
export class ZetlandDKArticleIE extends InfoExtractor {
  static override readonly _VALID_URL =
    String.raw`https?://www\.zetland\.dk/\w+/(?<id>(?<story_id>\w{8})-(?<uploader_id>\w{8})-(?:\w{5}))`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const nextData =
      this.searchNextjsData<Record<string, unknown>>(webpage, displayId) ?? {};
    const pageProps = getRecord(getRecord(nextData.props).pageProps);
    const storyData = getRecord(
      getRecord(getRecord(getRecord(pageProps.initialState).consume).story)
        .story,
    );
    ...
    const formats = asArray(storyMeta.audioFiles).flatMap((audioUrl) => {
    ...
    return mergeDicts(
      {
        ...
        title:
          stringValue(storyContentContent.title) ??
          stringValue(storyData.title),
        ...
        release_timestamp:
          unifiedTimestamp(storyData.published_at) ?? undefined,
        modified_timestamp: unifiedTimestamp(storyData.revised_at) ?? undefined,
      },
      {
        title:
          stringValue(meta.title) ??
          stringValue(ld.headline) ??
          stringValue(og["og:title"]) ??
          stringValue(og["twitter:title"]),
        description:
          stringValue(meta.description) ??
          stringValue(ld.description) ??
          stringValue(og["og:description"]) ??
          stringValue(og["twitter:description"]),
        uploader: stringValue(meta.author) ?? stringValue(author.name),
        uploader_url: urlOrNone(author.url) ?? undefined,
        thumbnail:
          urlOrNone(ld.image) ??
          urlOrNone(og["og:image"]) ??
          urlOrNone(og["twitter:image"]) ??
          undefined,
      ...
      },
      {
        ...
        thumbnail:
          this.htmlSearchMeta(["og:image", "twitter:image"], webpage) ??
          undefined,
        uploader: this.htmlSearchMeta(["author"], webpage) ?? undefined,
        release_timestamp:
          unifiedTimestamp(
            this.htmlSearchMeta(["article:published_time"], webpage),
          ) ?? undefined,
      },
      this.searchJsonLd(webpage, displayId, { defaultValue: {} }),
    ) as ExtractorInfo;
  }
}
