...
export class YouPornIE extends InfoExtractor {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?youporn\.com/(?:watch|embed)/(?<id>\d+)(?:/(?<display_id>[^/?#&]+))?/?(?:[#?]|$)`;
  static override readonly _EMBED_REGEX = [
    String.raw`<iframe[^>]+\bsrc=["'](?<url>(?:https?:)?//(?:www\.)?youporn\.com/embed/\d+)`,
  ];
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    if (!videoId) {
      throw new ExtractorError("Unable to extract YouPorn video id", {
        expected: true,
      });
    }
    const webpage = await this.downloadWebpage(
      `https://www.youporn.com/watch/${videoId}`,
      videoId,
    );
    if (webpage === false) {
      throw new Error("Unable to download YouPorn webpage");
    }
    const watchable = this.searchRegex(
      String.raw`(<div\s[^>]*\bid\s*=\s*('|")?watch-container(?(2)\2|(?!-)\b)[^>]*>)`,
      webpage,
      "watchability",
      { defaultValue: null },
    );
    if (!watchable) {
      const message = cleanHtml(getElementById("mainContent", webpage))?.split(
        /\s{2}/,
      )[0];
      throw new ExtractorError(
        message ? `${this.IE_NAME} says: ${message}` : "Video unavailable",
    ...
    const getFormatData = async (streamType: string): Promise<JsonRecord[]> => {
      const infoUrl = definitions
        .map((definition) =>
          definition.format === streamType
            ? urlOrNone(definition.videoUrl)
            : null,
        )
      ...
      const response = (await this.downloadJson<unknown[]>(infoUrl, videoId, {
        note: `Downloading ${streamType} info JSON`,
        fatal: false,
      })) as unknown[] | false | null;
      return response
        ? records(response).filter(
            (item) => item.format === streamType && urlOrNone(item.videoUrl),
          )
        : [];
    };
    ...
    for (const hls of await getFormatData("hls")) {
      ...
      if (
        hlsUrl &&
        hls.defaultQuality !== true &&
        hls.defaultQuality !== false
      ) {
        formats.push(
          ...this.extractM3u8Formats(hlsUrl, videoId, "mp4", { m3u8Id: "hls" }),
        );
      }
      for (const child of records(hls.videoUrl)) {
        ...
        if (childUrl) {
          formats.push(
            ...this.extractM3u8Formats(childUrl, videoId, "mp4", {
              m3u8Id: "hls",
            }),
          );
        }
    ...
    for (const definition of await getFormatData("mp4")) {
      ...
      const bitrateMatch =
        /(?<height>\d{3,4})[pP]_(?<bitrate>\d+)[kK]_\d+/.exec(videoUrl);
    ...
    const title =
      this.htmlSearchRegex(
        String.raw`(?s)<div[^>]+class=["']watchVideoTitle[^>]+>(.+?)</div>`,
        webpage,
        "title",
        { defaultValue: null },
      ) ??
    ...
    const description =
      this.htmlSearchRegex(
        String.raw`(?s)<div[^>]+\bid=["']description["'][^>]*>(.+?)</div>`,
        webpage,
        "description",
        { defaultValue: null },
      ) ?? this.ogSearchDescription(webpage);
    const thumbnail = this.searchRegex(
      String.raw`(?:imageurl\s*=|poster\s*:)\s*(["'])(?<thumbnail>.+?)\1`,
      webpage,
      "thumbnail",
      { fatal: false, group: "thumbnail" },
    );
    const duration =
      intOrNone(playerVars.duration) ??
      intOrNone(
        this.htmlSearchMeta("video:duration", webpage, "duration", false),
      );
    const uploader = this.htmlSearchRegex(
      String.raw`(?s)<div[^>]+class=["']submitByLink["'][^>]*>(.+?)</div>`,
      webpage,
      "uploader",
      { fatal: false },
    );
    const uploadDate = unifiedStrdate(
      this.htmlSearchRegex(
        [
    ...
    const views = this.searchRegex(
      String.raw`(<div [^>]*\bdata-value\s*=[^>]+>)\s*<label>Views:</label>`,
      webpage,
      "views",
      { defaultValue: null },
    );
    const viewCount =
      typeof views === "string"
        ? parseCount(extractAttributes(views)["data-value"] ?? null)
        : null;
    const commentCount = parseCount(
      this.searchRegex(
        String.raw`>All [Cc]omments? \(([\d,.]+)\)`,
        webpage,
        "comment count",
        { defaultValue: null },
      ) as string | null,
    ...
    const data = this.searchJsonLd(webpage, videoId, {
      defaultValue: {},
    }) as JsonRecord;
    delete data.url;
    const result = mergeDicts(data, {
      ...
      categories: extractTagBox(
        webpage,
        String.raw`(?s)Categories:.*?</[^>]+>(.+?)</div>`,
      ),
      tags: extractTagBox(
        webpage,
        String.raw`(?s)Tags:.*?</div>\s*<div[^>]+class=["']tagBoxContent["'][^>]*>(.+?)</div>`,
      ),
    ...
    if (
      ...
      result.description.startsWith(`Watch ${String(result.title)} online`)
    ) {
...
abstract class YouPornListBaseIE extends InfoExtractor {
  protected getNextUrl(url: string, html: string): string | null {
    ...
    const href = this.searchRegex(
      String.raw`<a [^>]*?\bhref\s*=\s*("|')(?<url>(?:(?!\1)[^>])+)\1`,
      next,
      "next page",
      { group: "url", defaultValue: null },
    );
  ...
  protected async entries(
    ...
    for (let page = start; page <= start; page += 1) {
      currentHtml ??=
        (await this.downloadWebpage(currentUrl, playlistId, {
          note: `Downloading page ${page}`,
          fatal: page === start,
        })) || null;
      if (!currentHtml) {
        return entries;
      }
      for (const element of getElementsHtmlByClass(
        "video-title",
        currentHtml,
      )) {
        const href = extractAttributes(element).href;
        const videoUrl =
          typeof href === "string" ? urljoin(currentUrl, href) : null;
        if (videoUrl) {
  ...
  protected async extractPlaylist(
    url: string,
    html: string | null = null,
  ): Promise<ExtractorInfo> {
    const match = this.matchValidUrl(url);
    ...
    const qs = Object.fromEntries(
      Object.entries(parseQs(url)).flatMap(([key, value]) =>
        value.length ? [[key, value.at(-1) ?? ""]] : [],
      ),
    );
    let title = (this.constructor as typeof YouPornListBaseIE).getTitleFromSlug(
      playlistId,
    );
    if (pageType) {
    ...
    const filters = Object.entries(qs)
      .sort()
    ...
    return this.playlistResult(
      await this.entries(url, playlistId, html, intOrNone(qs.page)),
      playlistId,
      title,
    );
  }
...
export class YouPornCategoryIE extends YouPornListBaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?youporn\.com/(?<type>category)/(?<id>[^/?#&]+)(?:/(?<sort>popular|views|rating|time|duration))?/?(?:[#?]|$)`;
}

export class YouPornChannelIE extends YouPornListBaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?youporn\.com/(?<type>channel)/(?<id>[^/?#&]+)(?:/(?<sort>rating|views|duration))?/?(?:[#?]|$)`;
...
export class YouPornCollectionIE extends YouPornListBaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?youporn\.com/(?<type>collection)s/videos/(?<id>\d+)(?:/(?<sort>rating|views|time|duration))?/?(?:[#?]|$)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const infos = (
      cleanHtml(getElementByClass("collection-infos", html)) ?? ""
    ).replaceAll(/\s+/g, " ");
    const titleUploader =
      /^\s*Collection: (?<title>.+?) \d+ VIDEOS \d+ VIEWS \d+ days LAST UPDATED From: (?<uploader>[\w_-]+)/.exec(
        infos,
      );
    if (titleUploader?.groups?.title) {
      playlist.title = String(playlist.title ?? "").replace(
        String(playlist.id ?? "").split("/")[0] ?? playlistId,
        titleUploader.groups.title,
      );
...
export class YouPornTagIE extends YouPornListBaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?youporn\.com/porn(?<type>tag)s/(?<id>[^/?#&]+)(?:/(?<sort>views|rating|time|duration))?/?(?:[#?]|$)`;

  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    this.reportWarning(
      "YouPorn tag pages are not correctly cached and often return incorrect results",
      null,
      true,
    );
...
export class YouPornStarIE extends YouPornListBaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?youporn\.com/(?<type>pornstar)/(?<id>[^/?#&]+)(?:/(?<sort>rating|views|duration))?/?(?:[#?]|$)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const playlist = await this.extractPlaylist(url, html);
    const infoMatch =
      /<div [^>]*\bclass\s*=\s*('|")(?:[\w$-]+\s+|\s)*?pornstar-info-wrapper(?:\s+[\w$-]+|\s)*\1[^>]*>(?<info>[\s\S]+?)(?:<\/div>\s*){6,}/.exec(
        html,
      );
    const info = infoMatch?.groups?.info
      ? cleanHtml(infoMatch.groups.info.replaceAll("\n", "nl=nl"))
          ?.replaceAll(/(?:\s*nl=nl)+\s*/g, " ")
          .replaceAll(/(?<=\s)\s+/g, " ")
          .replace("ribe Subsc", "")
      : null;
...
export class YouPornVideosIE extends YouPornListBaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?youporn\.com/(?:(?<id>browse)/)?(?<sort>(?:duration|rating|time|views|most_(?:favou?rit|view)ed|recommended|top_rated)?)?(?:[/#?]|$)`;
...
function extractTagBox(webpage: string, pattern: string): string[] {
  ...
  return [...match[1].matchAll(/<a[^>]+href=[^>]+>([^<]+)/g)]
    .map((item) => item[1] ?? "")
    .filter(Boolean);
}
