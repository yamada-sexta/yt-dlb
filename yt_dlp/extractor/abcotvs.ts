...
const ABCOTVSImageSchema = z.object({
  source: z.string().optional(),
  dynamicSource: z.string().optional(),
}).passthrough();

const ABCOTVSVideoSchema = z.object({
...
const ABCOTVSContentSchema = z.object({
...
const ABCOTVSClipSchema = z.object({
...
export class ABCOTVSIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://(?<site>abc(?:7(?:news|ny|chicago)?|11|13|30)|6abc)\.com(?:(?:/[^/]+)*/(?<display_id>[^/]+))?/(?<id>\d+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const rawData = await this.downloadJson<unknown>("https://api.abcotvs.com/v2/content", displayId, {
      query: {
        id: urlVideoId,
    ...
    const videoId = String(firstDefined(video.id, video.publishedKey, urlVideoId));
    ...
    if (m3u8Url) {
      formats.push(...this.extractM3u8Formats(m3u8Url.split("?")[0] ?? m3u8Url, displayId, "mp4", {
        m3u8Id: "hls",
      }));
    }
    if (video.mp4) {
    ...
    return {
      ...
      title,
      description: firstDefined(video.description, video.caption, video.meta?.description, null),
      thumbnail: firstDefined(video.image?.source, video.image?.dynamicSource, null),
...
export class ABCOTVSClipsIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://clips\.abcotvs\.com/(?:[^/]+/)*video/(?<id>\d+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const rawData = await this.downloadJson<unknown>(`https://clips.abcotvs.com/vogo/video/getByIds?ids=${videoId}`, videoId);
