...
import { determineExt, floatOrNone, intOrNone, joinNonempty, mimetype2ext, tryGet, urljoin } from "../utils/index.ts";
...
export class YandexDiskIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`(?x)https?://(?<domain>yadi\.sk|disk\.(?:360\.)?yandex\.(?:az|by|co(?:m(?:\.(?:am|ge|tr))?|\.il)|ee|fr|k[gz]|l[tv]|md|t[jm]|u[az]|ru))/(?:[di]/|public.*?\bhash=)(?<id>[^/?#&]+)`.replace(/\(\?x\)|\s+#.*|\s+/g, "");
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const store = typeof storeJson === "string" ? this.parseJson<Record<string, unknown>>(storeJson, videoId) ?? {} : {};
    ...
    const yandexuid = typeof environment.yandexuid === "string" ? environment.yandexuid : null;
    if (sk && yandexuid && (!sourceUrl || !Object.keys(videoStreams).length)) {
      const cookieJar = this.downloader?.cookies as { setCookie?: (cookie: Cookie) => void } | undefined;
      cookieJar?.setCookie?.(new Cookie({ name: "yandexuid", value: yandexuid, domain }));
      const apiBase = urljoin(url, "/public/api/");
      const callApi = async (action: string): Promise<Record<string, unknown>> => {
        if (!apiBase) {
          return {};
        }
        const response = await this.downloadJson<{ data?: Record<string, unknown> }>(`${apiBase}${action}`, videoId, {
      ...
      if (!sourceUrl) {
        ...
        sourceUrl = typeof downloadUrlData.url === "string" ? downloadUrlData.url : undefined;
      }
    ...
    if (sourceUrl) {
      formats.push({
        url: sourceUrl,
        format_id: "source",
        ext: determineExt(title, String(meta.ext ?? mimetype2ext(typeof meta.mime_type === "string" ? meta.mime_type : null) ?? "mp4")),
        quality: 1,
        filesize: intOrNone(meta.size),
      });
    }
    const videos = Array.isArray(videoStreams.videos) ? videoStreams.videos : [];
    for (const item of videos) {
      ...
      if (video.dimension === "adaptive") {
        formats.push(...this.extractM3u8Formats(formatUrl, videoId, "mp4", { entryProtocol: "m3u8_native", m3u8Id: "hls" }));
      } else {
    ...
    const displayName = uid ? tryGet(store, (value) => record(record(record(value).users)[uid]).displayName, (value): value is string => typeof value === "string") : null;
    return {
      id: videoId,
      title,
