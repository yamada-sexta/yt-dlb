...
export class YleAreenaIE extends InfoExtractor {
  static override readonly _VALID_URL =
    String.raw`https?://areena\.yle\.fi/(?<podcast>podcastit/)?(?<id>[\d-]+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    if (!videoId) {
      throw new ExtractorError("Unable to extract Yle Areena id", {
        expected: true,
      });
    }
    ...
    if (webpage === false) {
      throw new ExtractorError("Unable to download Yle Areena webpage", {
        videoId,
      });
    }
    const jsonLd = record(
      this.searchJsonLd(webpage, videoId, { defaultValue: {} }),
    );
    ...
    const episodeMatch =
      /K(?<season_no>\d+),\s*J(?<episode_no>\d+):?\s*\b(?<episode>[^|]+)\s*\|\s*(?<series>.+)/.exec(
        stringValue(jsonLd.title) ?? "",
      );
    const description = stringValue(
      nested(videoData, ["ongoing_ondemand", "description", "fin"]),
    );
    const subtitles: Record<string, Array<Record<string, unknown>>> = {};
    const subtitleItems = Array.isArray(
      nested(videoData, ["ongoing_ondemand", "subtitles"]),
    )
    ...
    if (isPodcast && urlOrNone(ondemand.media_url)) {
      ...
      info.url = smuggleUrl(`kaltura:1955031:${kalturaId}`, {
        source_url: url,
      });
      info.ie_key = "Kaltura";
    } else if (Object.keys(record(videoData.gone)).length) {
      this.raiseNoFormats("The content is no longer available", {
        expected: true,
        videoId,
      });
    } else {
    ...
    if (!info.url && typeof metadata.manifest_url === "string") {
      const [formats, manifestSubtitles] =
    ...
    const metaDescription = stringValue(
      nested(metadata, ["description", "fin"]),
    );
    return {
      id: videoId,
      title:
        metaTitle ??
        stringValue(episodeMatch?.groups?.episode) ??
        stringValue(jsonLd.title) ??
        videoId,
      description: metaDescription ?? description ?? undefined,
      series:
        stringValue(nested(metadata, ["series", "title", "fin"])) ??
        stringValue(episodeMatch?.groups?.series) ??
        undefined,
      season_number:
        intOrNone(
          this.searchRegex(
        ...
        intOrNone(episodeMatch?.groups?.season_no) ??
        undefined,
      episode_number:
        intOrNone(metadata.episode_number) ??
        intOrNone(episodeMatch?.groups?.episode_no) ??
        undefined,
      age_limit:
        intOrNone(nested(metadata, ["content_rating", "age_restriction"])) ??
        undefined,
      release_timestamp:
        parseIso8601(stringValue(metadata.start_time)) ?? undefined,
      duration:
        intOrNone(nested(metadata, ["duration", "duration_in_seconds"])) ??
        undefined,
      thumbnails: Array.isArray(jsonLd.thumbnails)
        ? jsonLd.thumbnails
        : undefined,
      ...info,
    };
