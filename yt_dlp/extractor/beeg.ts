...
export class BeegIE extends InfoExtractor {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?beeg\.(?:com(?:/video)?)/-?(?<id>\d+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const firstFact =
      [...(video.fc_facts ?? [])].sort(
        (left, right) =>
          (intOrNone(left.id) ?? Number.MAX_SAFE_INTEGER) -
          (intOrNone(right.id) ?? Number.MAX_SAFE_INTEGER),
      )[0] ?? {};
    const resources =
      video.file?.hls_resources ?? firstFact.hls_resources ?? {};
    ...
    for (const [formatId, videoUri] of Object.entries(resources)) {
      ...
      const height = intOrNone(
        this.searchRegex(/fl_cdn_(\d+)/, formatId, "height", {
          defaultValue: null,
        }),
      );
      for (const format of this.extractM3u8Formats(
        `https://video.beeg.com/${videoUri}`,
        videoId,
        "mp4",
        { m3u8Id: strOrNone(height) ?? undefined },
      )) {
