...
export class AlphaPornoIE extends InfoExtractor {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?alphaporno\.com/videos/(?<id>[^/]+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const videoId = this.searchRegex(
      /video_id\s*:\s*'([^']+)'/,
      webpage,
      "video id",
      { defaultValue: null },
    ...
    const extMeta =
      this.htmlSearchMeta("encodingFormat", webpage, "ext") ?? ".mp4";
    ...
    const keywords =
      this.htmlSearchMeta("keywords", webpage, "categories") ?? "";
    ...
    return {
      ...
      timestamp: parseIso8601(
        this.htmlSearchMeta("uploadDate", webpage, "upload date"),
      ),
      duration: parseDuration(
        this.htmlSearchMeta("duration", webpage, "duration"),
      ),
      filesize_approx: parseFilesize(
        this.htmlSearchMeta("contentSize", webpage, "file size"),
      ),
