...
export abstract class InfoExtractor {
    ...
    try {
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          ...
          this.toScreen(
            `Extracting URL: ${this.getParam("verbose", false) ? url : truncateString(url, 100, 20)}`,
          );
          ...
          if (
            error instanceof GeoRestrictedError &&
            this.maybeFakeIpAndRetry(error.countries)
          ) {
            continue;
          }
          throw error;
      ...
      if (error instanceof ExtractorError) {
        error.video_id ||= (this.constructor as typeof InfoExtractor).getTempId(
          url,
        );
  ...
  protected async getNetrcLoginInfo(
    netrcMachine: string | null = null,
  ): Promise<[string | null, string | null]> {
    const machine =
      netrcMachine ??
      ((this.constructor as typeof InfoExtractor)._NETRC_MACHINE || null);
    if (!machine) {
      throw new ExtractorError(
        `Missing netrc_machine and ${this.constructor.name}._NETRC_MACHINE`,
      );
    }
    if (
      machine.startsWith("-") ||
      machine.startsWith("_") ||
      !/^[A-Za-z0-9._-]+$/.test(machine)
    ) {
      throw new ExtractorError(
        `Invalid netrc machine: ${JSON.stringify(machine)}`,
        { expected: true },
      );
    }
  ...
  protected _get_netrc_login_info(
  ...
  protected async getLoginInfo(
    netrcMachine: string | null = null,
  ): Promise<[string | null, string | null]> {
    const username =
      this.getParam<string | null>("username", null) ??
      this.getParam<string | null>("ap_username", null);
    const password =
      this.getParam<string | null>("password", null) ??
      this.getParam<string | null>("ap_password", null);
  ...
  protected getTfaInfo(note = "two-factor verification code"): string | null {
    return (
      this.getParam<string | null>("twofactor", null) ??
      this.getParam<string | null>("2fa", null) ??
      this.getParam<string | null>(note, null)
    );
  }

  protected raiseLoginRequired(
    message = "This video requires login",
    options: { metadataAvailable?: boolean; method?: string | null } = {},
  ): never {
    void options;
    throw new ExtractorError(message, { expected: true });
  }

  protected raiseNoFormats(
    message: string,
    options: { expected?: boolean; videoId?: string | null } = {},
  ): never {
  ...
  protected raiseGeoRestricted(
    message = "This video is not available from your location",
    options: {
    ...
    throw new GeoRestrictedError(
      message,
      options.countries ??
        (this.constructor as typeof InfoExtractor)._GEO_COUNTRIES,
    );
  }
  ...
  protected getCookies(url: string): LenientSimpleCookie {
    ...
    if (
      jar &&
      typeof jar === "object" &&
      "getCookieHeader" in jar &&
      typeof jar.getCookieHeader === "function"
    ) {
      const getCookieHeader = jar.getCookieHeader as (
        url: string,
      ) => string | undefined;
  ...
  protected formHiddenInputs(
    ...
    new HTMLRewriter()
      .on("form", {
        element(element) {
          if (
            element.getAttribute("id") === formId ||
            element.getAttribute("name") === formId
          ) {
            inForm = true;
            element.onEndTag(() => {
              inForm = false;
  ...
  protected async requestWebpage(
    ...
    try {
      ...
      if (
        !response.ok &&
        !canAcceptStatusCode(response.status, options.expected_status ?? null)
      ) {
        throw new ExtractorError(
      ...
      if (fatal) {
        throw error instanceof ExtractorError
          ? error
          : new ExtractorError(
              `${videoId}: ${errnote || "Unable to download webpage"}`,
              { cause: error, videoId },
            );
      }
      if (errnote) {
        this.reportWarning(
          `${videoId}: ${errnote}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
      return false;
    }
  ...
  protected async downloadWebpageHandle(
    ...
    return [
      await decodeResponseText(response, options.encoding ?? null),
      response,
    ];
  }
  ...
  protected async downloadWebpage(
    ...
    const result = await this.downloadWebpageHandle(
      urlOrRequest,
      videoId,
      options,
    );
  ...
  protected async downloadJson<T = unknown>(
    urlOrRequest: string | URL | Request,
    videoId: string,
    options: DownloadOptions & { fatal: false },
  ...
  protected async downloadJson<T = unknown>(
    ...
    try {
      return JSON.parse(
        options.transform_source
          ? String(options.transform_source(webpage))
          : webpage,
      ) as T;
    } catch (error) {
      ...
      throw new ExtractorError("Failed to parse JSON", {
        cause: error,
        videoId,
      });
    }
  ...
  protected async downloadXml(
    ...
    try {
      ...
      throw new ExtractorError("Failed to parse XML", {
        cause: error,
        videoId,
      });
    }
  ...
  protected searchRegex(
    ...
    name: string,
    options: {
      defaultValue?: string | typeof NO_DEFAULT | null;
      fatal?: boolean;
      group?: string | number | readonly (string | number)[];
    } = {},
  ): string | string[] | null {
    const fatal = options.fatal ?? true;
  ...
  protected searchJson<T = unknown>(
    ...
    const jsonString = this.searchRegex(
      ...
      name,
      {
        ...
        defaultValue: options.defaultValue === undefined ? NO_DEFAULT : null,
      },
    );
    if (typeof jsonString !== "string") {
      return options.defaultValue === NO_DEFAULT ||
        options.defaultValue === undefined
        ? null
        : options.defaultValue;
    }
    const transformed = options.transform_source
      ? options.transform_source(jsonString)
      : jsonString;
    if (transformed === null) {
      return options.defaultValue === NO_DEFAULT ||
        options.defaultValue === undefined
    ...
    try {
      ...
      if (options.fatal === false) {
        this.reportWarning(
          `Unable to extract ${name} - Failed to parse JSON: ${error instanceof Error ? error.message : String(error)}`,
          videoId,
        );
        return options.defaultValue === NO_DEFAULT ||
          options.defaultValue === undefined
      ...
      throw new ExtractorError(
        `Unable to extract ${name} - Failed to parse JSON`,
        { cause: error, videoId },
      );
    }
  }
      ...
      .on("meta", {
        element(element) {
          ...
          if (index >= 0 && index < bestIndex) {
            bestIndex = index;
            result = normalizeMetaContentAttribute(
              webpage,
  ...
  protected searchJsonLd(
    ...
    for (const json of collectScriptText(
      webpage,
  ...
  protected searchNextjsData<T = unknown>(
    webpage: string,
    videoId: string,
  ...
  protected searchNextjsV13Data(
    ...
    for (const scriptText of collectScriptText(webpage, "script")) {
      ...
      const segment = this.parseJson<unknown>(
        source.slice("self.__next_f.push(".length, -1),
        videoId ?? "",
        { fatal },
      );
      if (!Array.isArray(segment) || segment.length !== 2) {
        this.writeDebug(
          `${videoId ?? "unknown"}: Unsupported next.js flight data structure detected`,
        );
        continue;
      }
    ...
    for (const line of flightText.split(/\r?\n/)) {
      ...
      if (body.startsWith("[") && body.endsWith("]")) {
        flattenNextjsFlightData(
          this.parseJson<unknown>(body, videoId ?? "", { fatal: false }),
          nextjsData,
        );
      } else if (body.startsWith("{") && body.endsWith("}")) {
        const data = this.parseJson<unknown>(body, videoId ?? "", {
          fatal: false,
        });
        if (data !== null) {
  ...
  protected _search_nextjs_v13_data(
    webpage: string | null | undefined,
    videoId: string | null,
    fatal = true,
  ): Record<string, unknown> {
  ...
  protected searchNuxtJson<T = unknown>(
    ...
    return this.resolveNuxtArray<T>(checked.data, videoId, {
      fatal,
      defaultValue: passedDefault ? (fallback as T) : NO_DEFAULT,
    });
  }
  ...
  protected searchNuxtData<T = Record<string, unknown>>(
    ...
    const pattern1 = new RegExp(
      `<script>\\s*window\\.${reCtx}\\s*=\\s*${functionRe}\\s*;?\\s*</script>`,
      "s",
    );
    ...
    const valsParsed =
      this.parseJson<unknown[]>(`[${argVals}]`, videoId, {
    ...
    return (options.traverse
      ? traverseObj(parsedJs, options.traverse)
      : parsedJs) as unknown as T;
  }
  ...
  protected _search_nuxt_data<T = Record<string, unknown>>(
    ...
    return this.searchNuxtData<T>(webpage, videoId, {
      contextName,
      ...options,
    });
  }
  ...
  protected resolveNuxtArray<T = unknown>(
    ...
    const passedDefault =
      options.defaultValue !== undefined && options.defaultValue !== NO_DEFAULT;
    const fallback = passedDefault ? (options.defaultValue as T) : undefined;
    const fatal = passedDefault ? false : (options.fatal ?? true);
    ...
    try {
      return (
        (resolveNuxtValue(array, 0, fatal) as T | Record<string, unknown>) ||
        (passedDefault ? (fallback as T) : {})
      );
    } catch (error) {
      if (fatal) {
        throw new ExtractorError("Unable to resolve Nuxt JSON data", {
          cause: error,
          videoId,
        });
      }
      ...
      this.reportWarning(
        `Error resolving Nuxt JSON: ${error instanceof Error ? error.message : String(error)}`,
        videoId,
        true,
      );
      return {};
  ...
  private nuxtJsonDefault<T>(
    ...
    this.reportWarning(
      "Unable to resolve Nuxt JSON data: invalid input",
      videoId,
      true,
    );
    return {};
  ...
  static urlResult(
    url: string,
    ie: string | typeof InfoExtractor | null = null,
    videoId: string | null = null,
  ...
  static playlistResult(
    entries: Iterable<ExtractorInfo> | ExtractorInfo[],
    playlistId: string | null = null,
    playlistTitle: string | null = null,
    playlistDescription: string | null = null,
    options: Record<string, unknown> = {},
  ): ExtractorInfo {
    return {
      ...options,
  ...
  protected playlistResult(
    ...
    return InfoExtractor.playlistResult(
      entries,
      playlistId,
      playlistTitle,
      playlistDescription,
  ...
  protected yesPlaylist(
    ...
    options: {
      smuggledData?: Record<string, unknown> | null;
    ...
    if (this.getParam("noplaylist", false)) {
      this.toScreen(
        `Downloading just the ${videoLabel}${videoIdStr} because of --no-playlist`,
      );
      return false;
    }
    this.toScreen(
      `Downloading ${playlistLabel}${playlistIdStr} - add --no-playlist to download just the ${videoLabel}${videoIdStr}`,
    );
    return true;
  }
  ...
  static playlistFromMatches(
    ...
    options: {
      ...
      ie?: string | typeof InfoExtractor | null;
      videoKwargs?: Record<string, unknown>;
  ...
  protected ogSearchProperty(
    property: string,
    webpage: string,
    fatal = true,
  ): string | null {
    const result = this.htmlSearchMeta(
      `og:${property}`,
      webpage,
      `OpenGraph ${property}`,
      fatal,
  ...
  protected rtaSearch(html: string): number | null {
    ...
    new HTMLRewriter()
      .on("meta", {
        element(element) {
          hasRtaMeta ||=
            element.getAttribute("name") === "rating" &&
    ...
    for (const marker of markers) {
      ...
      if (match) {
        ageLimit = Math.max(
          ageLimit ?? 0,
          Number.parseInt(match.groups?.age ?? "18", 10),
        );
      }
    }
    return ageLimit;
    ...
    for (const selector of mediaSelectors) {
      rewriter = rewriter.on(selector, {
        element: (element) => {
          ...
          if (src) {
            const [, formats] = this.html5MediaFormats(
              src,
              mediaType,
              baseUrl,
              videoId,
              parseHtml5ContentType(attrs.type),
              options,
            );
    ...
    rewriter
      .on("source", {
        element: (element) => {
          ...
          const [isPlainUrl, formats] = this.html5MediaFormats(
            src,
            media.mediaType,
            baseUrl,
            videoId,
          ...
          const format = {
            ...
            ...((attrs.label ?? attrs.title)
              ? { format_id: attrs.label ?? attrs.title }
              : {}),
          };
  ...
  private html5MediaFormats(
    ...
    const ext =
      typeof typeInfo.ext === "string" ? typeInfo.ext : determineExt(fullUrl);
    if (ext === "m3u8") {
      return [
        false,
        this.extractM3u8Formats(fullUrl, videoId ?? "", "mp4", {
    ...
    if (ext === "mpd") {
      return [
        false,
        this.extractMpdFormats(fullUrl, videoId ?? "", {
          mpdId: options.mpdId ?? undefined,
          fatal: false,
        }),
      ];
    }
  ...
  protected parseM3u8FormatsAndSubtitles(
      ...
      return [
        [
          filterUndefined({
            ...
            url:
              m3u8Url ??
              `data:application/x-mpegurl,${encodeURIComponent(m3u8Doc)}`,
            ext,
      ...
      if ((mediaType === "AUDIO" || mediaType === "VIDEO") && media.URI) {
        ...
        formats.push(
          filterUndefined({
            ...
            _audio_group_id:
              isAudio && media.DEFAULT !== "NO" && media.AUTOSELECT !== "NO"
                ? groupId
                : undefined,
          }),
        );
    ...
    for (const rawLine of m3u8Doc.split(/\r?\n/)) {
      ...
      const tbr = floatOrNone(
        lastStreamInf["AVERAGE-BANDWIDTH"] ?? lastStreamInf.BANDWIDTH,
        1000,
      );
      const resolution = lastStreamInf.RESOLUTION
        ? parseResolution(lastStreamInf.RESOLUTION)
        : {};
      const codecs = parseM3u8Codecs(lastStreamInf.CODECS);
      const formatId = options.live
        ? (options.m3u8Id ?? undefined)
        : joinNonempty(
            options.m3u8Id,
            lastStreamInf.NAME ?? String(tbr ?? formats.length),
          );
      const format: Record<string, unknown> = filterUndefined({
        ...
        ...codecs,
        _audio_group_id:
          lastStreamInf.AUDIO && codecs.vcodec !== "none"
            ? lastStreamInf.AUDIO
            : undefined,
      });
      if (
        lastStreamInf.AUDIO &&
        codecs.vcodec !== "none" &&
        groups[lastStreamInf.AUDIO]?.[0]?.URI
      ) {
        format.acodec = "none";
      }
      formats.push(format);
      ...
      ...new Set(
        formats
          .filter(
            (format) =>
              format.vcodec !== "none" &&
              typeof format._audio_group_id === "string",
          )
  ...
  protected _parse_m3u8_formats_and_subtitles(
  ...
  protected async extractMpdFormatsAndSubtitles(
    ...
    const [formats, subtitles] = this.parseMpdFormatsAndSubtitles(manifest, {
      mpdUrl,
    });
    if (options.mpdId) {
      for (const format of formats) {
        format.format_id = joinNonempty(
          options.mpdId,
          strOrNone(format.format_id),
        );
      }
    }
    return [formats, subtitles];
  ...
  protected parseMpdFormatsAndSubtitles(
    ...
    const root =
      typeof mpdDoc === "string" ? compatEtreeFromstring(mpdDoc) : mpdDoc;
    const formats: Array<Record<string, unknown>> = [];
    const subtitles: Record<string, Array<Record<string, unknown>>> = {};
    const rootBase =
      firstChildText(root, "BaseURL") ??
      normalizeMpdBaseUrl(options.mpdBaseUrl) ??
      options.mpdUrl ??
      null;

    for (const period of childrenByName(root, "Period")) {
      const periodBase =
        urljoin(rootBase, firstChildText(period, "BaseURL")) ?? rootBase;
      for (const adaptation of childrenByName(period, "AdaptationSet")) {
        const adaptationBase =
          urljoin(periodBase, firstChildText(adaptation, "BaseURL")) ??
          periodBase;
        const adaptationTemplate = firstChild(adaptation, "SegmentTemplate");
        for (const representation of childrenByName(
          adaptation,
          "Representation",
        )) {
          ...
          const representationBase =
            urljoin(
              adaptationBase,
              firstChildText(representation, "BaseURL"),
            ) ?? adaptationBase;
          ...
          const segmentTemplate =
            firstChild(representation, "SegmentTemplate") ?? adaptationTemplate;
          const common = filterUndefined({
            ...
            url: firstChild(representation, "BaseURL")
              ? representationBase
              : (options.mpdUrl ?? representationBase ?? undefined),
            fragment_base_url:
              segmentTemplate && adaptationBase ? adaptationBase : undefined,
          });
          if (contentType === "text") {
          ...
          formats.push(
            filterUndefined({
              ...common,
              format_note:
                contentType === "audio" ? "DASH audio" : "DASH video",
              container: contentType === "audio" ? "m4a_dash" : "mp4_dash",
              acodec:
                contentType === "audio"
                  ? (codecs.acodec ?? attrs.codecs)
                  : "none",
              vcodec:
                contentType === "audio"
                  ? "none"
                  : (codecs.vcodec ?? attrs.codecs),
              audio_ext:
                contentType === "audio"
                  ? "m4a"
                  : codecs.acodec
  ...
  protected _parse_mpd_formats_and_subtitles(
  ...
  protected parseIsmFormatsAndSubtitles(
    ...
    const root =
      typeof ismDoc === "string" ? compatEtreeFromstring(ismDoc) : ismDoc;
    ...
    for (const stream of childrenByName(root, "StreamIndex")) {
      ...
      for (const track of childrenByName(stream, "QualityLevel")) {
        const fourcc =
          track.attrib.FourCC ??
          { "255": "AACL", "65534": "EC-3" }[track.attrib.AudioTag ?? ""];
        if (
          !fourcc ||
          !["H264", "AVC1", "AACL", "TTML", "EC-3"].includes(fourcc)
        ) {
          continue;
        }
        ...
        const common = filterUndefined({
          ...
          _download_params: filterUndefined({
            ...
            bits_per_sample:
              intOrNone(track.attrib.BitsPerSample, 1, 16) ?? undefined,
            nal_unit_length_field:
              intOrNone(track.attrib.NALUnitLengthField, 1, 4) ?? undefined,
          }),
        });
        if (streamType === "text") {
          subtitles[language] ??= [];
  ...
  protected _parse_ism_formats_and_subtitles(
  ...
  protected parseF4mFormats(
    ...
    const root =
      typeof manifest === "string" ? compatEtreeFromstring(manifest) : manifest;
    ...
    return childrenByName(root, "media").flatMap((media, index) => {
      ...
      return [
        filterUndefined({
          ...
          vcodec: firstChildText(root, "mimeType")?.startsWith("audio/")
            ? "none"
            : undefined,
          preference: options.preference ?? undefined,
          quality: options.quality ?? undefined,
  ...
  protected parseXspf(
    ...
    const root =
      typeof xspfDoc === "string" ? compatEtreeFromstring(xspfDoc) : xspfDoc;
    const trackList = firstChild(root, "trackList");
    return childrenByName(trackList ?? root, "track")
      .map((track) => ({
        id: playlistId,
        ...
        duration:
          floatOrNone(firstChildText(track, "duration"), 1000) ?? undefined,
        formats: childrenByName(track, "location").flatMap((location) => {
          const formatUrl = urljoin(
            options.xspfBaseUrl ?? options.xspfUrl,
            location.text,
          );
          return formatUrl
            ? [
                filterUndefined({
                  ...
                  format_id:
                    location.attrib[
                      "{http://static.streamone.nl/player/ns/0}label"
                    ],
                  width:
                    intOrNone(
                      location.attrib[
                        "{http://static.streamone.nl/player/ns/0}width"
                      ],
                    ) ?? undefined,
                  height:
                    intOrNone(
                      location.attrib[
                        "{http://static.streamone.nl/player/ns/0}height"
                      ],
                    ) ?? undefined,
                }),
              ]
  ...
  protected findJwplayerData(
    ...
    const setupPattern =
      /\bjwplayer\s*\(\s*(["'])(?:(?!\1)[\s\S])+?\1\s*\)(?:(?!<\/script>)[\s\S])*?\.\s*(?:setup|load)\s*\(/g;
    for (const match of webpage.matchAll(setupPattern)) {
      ...
      try {
        ...
        this.reportWarning(
          `Unable to extract JWPlayer data - Failed to parse JSON: ${error instanceof Error ? error.message : String(error)}`,
          videoId ?? "",
        );
      }
    }
  ...
  protected extractJwplayerData(
    ...
    const jwplayerData = this.findJwplayerData(
      webpage,
      videoId,
      options.transformSource ?? jsToJson,
    );
    return this.parseJwplayerData(jwplayerData, videoId, options);
  }

  protected _extract_jwplayer_data(
  ...
  protected parseJwplayerData(
    ...
    const items = playlistItems.success
      ? playlistItems.data
      : [jwplayerData.playlist ?? jwplayerData];
    const entries: ExtractorInfo[] = [];
    for (const rawItem of items) {
      ...
      const sourceList = JsonArraySchema.safeParse(videoData.data.sources)
        .success
        ? JsonArraySchema.parse(videoData.data.sources)
        : [videoData.data];
      const thisVideoId =
        videoId ?? strOrNone(videoData.data.mediaid) ?? undefined;
      const formats = this.parseJwplayerFormats(
        sourceList,
        thisVideoId ?? null,
        {
          m3u8Id: options.m3u8Id,
      ...
      for (const rawTrack of jsonLdArray(videoData.data.tracks)) {
        ...
        const kind = strOrNone(
          track.success ? track.data.kind : null,
        )?.toLowerCase();
        if (!track.success || (kind !== "captions" && kind !== "subtitles")) {
          continue;
        }
        const trackUrl = urljoin(
          options.baseUrl,
          this.protoRelativeUrl(strOrNone(track.data.file)),
        );
        if (!trackUrl) {
          continue;
        }
      ...
      const entry: ExtractorInfo = {
        ...
        title:
          jsonLdString(
            requireTitle ? videoData.data.title : videoData.data.title,
          ) ?? undefined,
        description:
          cleanHtml(strOrNone(videoData.data.description)) ?? undefined,
        thumbnail:
          urljoin(
            options.baseUrl,
            this.protoRelativeUrl(strOrNone(videoData.data.image)),
          ) ?? undefined,
        timestamp: intOrNone(videoData.data.pubdate) ?? undefined,
        duration:
          floatOrNone(jwplayerData.duration ?? videoData.data.duration) ??
          undefined,
        subtitles,
        ...
        channel:
          cleanHtml(
            strOrNone(videoData.data.category ?? videoData.data.channel),
          ) ?? undefined,
      ...
      if (
        formats.length === 1 &&
        typeof formats[0]?.url === "string" &&
        /^(?:http|\/\/).*(?:youtube\.com|youtu\.be)\/.+/.test(formats[0].url)
      ) {
  ...
  protected _parse_jwplayer_data(
    ...args: Parameters<InfoExtractor["parseJwplayerData"]>
  ): ReturnType<InfoExtractor["parseJwplayerData"]> {
  ...
  protected parseJwplayerFormats(
    ...
    for (const rawSource of jwplayerSourcesData) {
      ...
      const sourceUrl = urljoin(
        options.baseUrl,
        this.protoRelativeUrl(strOrNone(source.data.file)),
      );
      ...
      if (
        sourceType === "hls" ||
        ext === "m3u8" ||
        sourceUrl.includes("format=m3u8-aapl")
      ) {
        formats.push(
          ...this.extractM3u8Formats(sourceUrl, videoId ?? "", "mp4", {
        ...
        sourceType === "dash" ||
        ext === "mpd" ||
        sourceUrl.includes("format=mpd-time-csf")
      ) {
        formats.push(
          ...this.extractMpdFormats(sourceUrl, videoId ?? "", {
            mpdId: options.mpdId,
            fatal: false,
          }),
        );
        ...
        formats.push(
          ...this.extractSmilFormats(sourceUrl, videoId ?? "", {
            fatal: false,
          }),
        );
      } else if (
        sourceType.startsWith("audio") ||
        ["oga", "aac", "mp3", "mpeg", "vorbis"].includes(ext)
      ) {
        formats.push({ url: sourceUrl, vcodec: "none", ext });
      } else {
        const formatId = strOrNone(source.data.label);
        const height =
          intOrNone(source.data.height) ??
          (formatId ? parseResolution(formatId).height : undefined);
        ...
        if (sourceUrl.startsWith("rtmp")) {
          format.ext = "flv";
          const rtmpParts =
            /^(?<url>.*?)(?<prefix>(?:mp4|mp3|flv):)(?<playPath>.*)$/.exec(
              sourceUrl,
            );
          if (rtmpParts?.groups) {
  ...
  protected _parse_jwplayer_formats(
    ...args: Parameters<InfoExtractor["parseJwplayerFormats"]>
  ): ReturnType<InfoExtractor["parseJwplayerFormats"]> {
  ...
  protected createRequest(
    ...
    const request =
      urlOrRequest instanceof Request
        ? urlOrRequest
        : new YtdlRequest(String(urlOrRequest));
  ...
  const decoder = new TextDecoder(
    (encoding ??
      guessEncoding(
        ...
        bytes,
      )) as ConstructorParameters<typeof TextDecoder>[0],
    { fatal: false },
  );
...
function guessEncoding(contentType: string | null, bytes: Uint8Array): string {
  const charset = /[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_.-]+\s*;\s*charset=(.+)/.exec(
    contentType ?? "",
  )?.[1];
  if (charset) {
    return charset;
  ...
  return (
    ...
    (bytes[0] === 0xff && bytes[1] === 0xfe ? "utf-16le" : "utf-8")
  );
}

function canAcceptStatusCode(
  status: number,
  expected: DownloadOptions["expected_status"],
): boolean {
...
function collectJsonLdCandidates(value: unknown): Record<string, unknown>[] {
  ...
  if (
    graph.success &&
    (Object.keys(object.data).length === 2 ||
      graph.data.some((item) => JsonObjectSchema.safeParse(item).success))
  ) {
...
function normalizeJsonLdInfo(
  ...
  if (jsonLdHasType(types, "TVEpisode", "Episode", "PodcastEpisode")) {
    ...
    if (
      season.success &&
      jsonLdIsType(season.data, "TVSeason", "Season", "CreativeWorkSeason")
    ) {
      setDefined(info, "season", jsonLdString(season.data.name));
      setDefined(info, "season_number", intOrNone(season.data.seasonNumber));
    }
    const series = JsonObjectSchema.safeParse(
      node.partOfSeries ?? node.partOfTVSeries,
    );
    if (
      series.success &&
      jsonLdIsType(series.data, "TVSeries", "Series", "CreativeWorkSeries")
    ) {
    ...
    setDefined(
      info,
      "timestamp",
      parseIso8601(jsonLdString(node.datePublished)),
    );
    setDefined(info, "title", jsonLdString(node.headline ?? node.name));
    setDefined(
      info,
      "description",
      jsonLdString(node.articleBody ?? node.description),
    );
  ...
  if (
    expectedType &&
    !jsonLdHasType(types, expectedType) &&
...
function extractJsonLdVideoObject(
  node: Record<string, unknown>,
  ...
  setDefined(
    info,
    "ext",
    mimetype2ext(jsonLdString(node.encodingFormat), undefined),
  );
  ...
  setDefined(
    info,
    "timestamp",
    parseIso8601(jsonLdString(node.uploadDate ?? node.datePublished)),
  );
...
function extractJsonLdInteractionStatistic(
  node: Record<string, unknown>,
  info: Record<string, unknown>,
): void {
  for (const statistic of jsonLdArray(node.interactionStatistic).flatMap(
    (item) => {
    ...
    const interactionType = JsonObjectSchema.safeParse(
      statistic.interactionType,
    );
    const actionType = interactionType.success
      ? jsonLdTypes(interactionType.data)
      : jsonLdTypes(statistic);
...
function extractJsonLdChapters(
  node: Record<string, unknown>,
  info: Record<string, unknown>,
): void {
  ...
  const chapters = clips.map((clip, index) => ({
    ...
    start_time:
      clip.start_time ??
      (index === 0 ? 0 : (clips[index - 1]?.end_time ?? null)),
    end_time:
      clip.end_time ??
      clips[index + 1]?.start_time ??
      (index === clips.length - 1 ? duration : null),
  }));
  if (
    chapters.every(
      (chapter) =>
...
function jsonLdTypes(node: Record<string, unknown>): string[] {
  return jsonLdArray(node["@type"]).flatMap((value) => {
    ...
    return [
      parsed.data.replace(/[/#]$/, "").split(/[/#]/).pop() ?? parsed.data,
    ];
  });
}

function jsonLdIsType(
  node: Record<string, unknown>,
  ...types: string[]
): boolean {
...
function parseM3u8Codecs(
  ...
  const parts = codecs
    ...
    .filter(Boolean);
  const video = parts.find((codec) =>
    /^(?:avc|hev|hvc|vp0?[89]|av01|theora)/i.test(codec),
  );
  const audio = parts.find((codec) =>
    /^(?:mp4a|ac-?3|ec-?3|opus|vorbis|flac)/i.test(codec),
  );
  if (video) {
    return {
      vcodec: video,
...
function parseMpdCodecs(codecs: string | null | undefined): {
  acodec?: string;
  vcodec?: string;
} {
...
function parseNetrcAuthenticators(
  content: string,
  machine: string,
): [string, string] | null {
...
function flattenNextjsFlightData(
  flightData: unknown,
  nextjsData: Record<string, unknown>,
): void {
    ...
    if (value && typeof value === "object") {
      return Object.fromEntries(
        Object.entries(value as Record<string, unknown>).map(([key, item]) => [
          key,
          resolveNuxtValue(array, item, fatal, resolving),
        ]),
      );
    }
    return value;
...
function resolveNuxtArrayValue(
  array: unknown[],
  value: unknown[],
  fatal: boolean,
...
function attrsFromElement(
  element: HTMLRewriterTypes.Element,
): Record<string, string> {
  return Object.fromEntries(
    [...element.attributes].map(([key, value]) => [key.toLowerCase(), value]),
  );
}

function absoluteHtml5Url(
  baseUrl: string,
...
function parseHtml5ContentType(
  contentType: string | null | undefined,
): Record<string, unknown> {
  if (!contentType) {
    return {};
  }
  const match = /(?<mimetype>[^/]+\/[^;]+)(?:;\s*codecs="?([^"]+))?/i.exec(
    contentType,
  );
...
function normalizeMetaContentAttribute(
  webpage: string,
  value: string | null,
): string | null {
  ...
  if (
    webpage.includes(`content="${value}"`) ||
    webpage.includes(`content='${value}'`)
  ) {
    return value;
  }
