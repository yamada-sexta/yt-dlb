...
export class YappyIE extends InfoExtractor {
  ...
  static override readonly _VALID_URL =
    String.raw`https?://yappy\.media/video/(?<id>\w+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const nextjsData = record(
      this.searchNextjsData(webpage, videoId, { defaultValue: {} }),
    );
    ...
    if (!Object.keys(mediaData).length) {
      const downloaded = (await this.downloadJson<Record<string, unknown>>(
        `https://yappy.media/api/video/${videoId}`,
        videoId,
    ...
    return {
      ...
      formats,
      thumbnail:
        stringValue(mediaData.thumbnail) ??
        this.htmlSearchMeta(["og:image", "og:image:secure_url"], webpage) ??
        undefined,
      description:
        stringValue(mediaData.description) ??
        stringValue(jsonLd.description) ??
        this.htmlSearchMeta(["description", "og:description"], webpage) ??
        undefined,
      timestamp:
        unifiedTimestamp(mediaData.publishedAt ?? jsonLd.timestamp) ??
        undefined,
      view_count:
        intOrNone(mediaData.viewsCount ?? jsonLd.view_count) ?? undefined,
      like_count: intOrNone(mediaData.likesCount) ?? undefined,
      uploader:
        stringValue(nested(mediaData, ["creator", "firstName"])) ?? undefined,
      uploader_id:
        stringValue(nested(mediaData, ["creator", "uuid"])) ??
        stringValue(nested(mediaData, ["creator", "nickname"])) ??
        undefined,
      categories,
...
export class YappyProfileIE extends InfoExtractor {
  static override readonly _VALID_URL =
    String.raw`https?://yappy\.media/profile/(?<id>\w+)`;

  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    const profileId = this.matchId(url);
    const entries: ExtractorInfo[] = [];
    for (let page = 1; page <= 1; page += 1) {
      const videos = (await this.downloadJson<Record<string, unknown>>(
        `https://yappy.media/api/video/list/${profileId}?page=${page}`,
        profileId,
        { note: `Downloading profile page ${page} JSON` },
      )) as Record<string, unknown> | false;
      const results =
