...
export class YoutubeNotificationsIE extends YoutubeTabBaseInfoExtractor {
  ...
  protected *extractNotificationMenu(response: unknown, continuationList: Array<Record<string, unknown> | null>): Iterable<ExtractorInfo> {
    continuationList[0] = null;
    const notificationList = firstArray([
      traverseObj<unknown>(response, ["actions", 0, "openPopupAction", "popup", "multiPageMenuRenderer", "sections", 0, "multiPageMenuNotificationSectionRenderer", "items"], { get_all: false }),
      traverseObj<unknown>(response, ["actions", 0, "appendContinuationItemsAction", "continuationItems"], { get_all: false }),
    ]);
  ...
  protected _extract_notification_menu(response: unknown, continuationList: Array<Record<string, unknown> | null>): Iterable<ExtractorInfo> {
    return this.extractNotificationMenu(response, continuationList);
  }

  protected async notificationMenuEntries(ytcfg: Record<string, unknown>): Promise<ExtractorInfo[]> {
    ...
    for (let page = 1; ; page += 1) {
      const ctoken = nestedString(continuationList[0], ["continuationEndpoint", "getNotificationMenuEndpoint", "ctoken"]);
      const response = await this.callApi<unknown>("notification/get_notification_menu", ctoken ? { ctoken } : {}, `page ${page}`, {
      ...
      entries.push(...this.extractNotificationMenu(parsedResponse.data, continuationList));
  ...
  protected _notification_menu_entries(ytcfg: Record<string, unknown>): Promise<ExtractorInfo[]> {
  ...
  protected override async realExtract(_url: string): Promise<ExtractorInfo | null> {
    ...
    return this.playlistResult(await this.notificationMenuEntries({}), displayId, displayId);
  }
}
