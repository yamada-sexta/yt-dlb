...
export class YoutubeTabBaseInfoExtractor extends YoutubeBaseInfoExtractor {
  protected extractBasicItemRenderer(
    item: unknown,
    ...
    for (const [key, renderer] of Object.entries(item)) {
      ...
      if (
        knownBasicRenderers.has(key) ||
        (key.startsWith("grid") && key.endsWith("Renderer"))
      ) {
        return renderer;
      }
    }
  ...
  protected extractVideo(
    ...
    const description = this.getText(renderer, "descriptionSnippet", [
      "detailedMetadataSnippets",
      Ellipsis,
      "snippetText",
    ]);
    const duration =
      intOrNone(renderer.lengthSeconds) ??
      parseDuration(
        this.getText(renderer, "lengthText", [
          "thumbnailOverlays",
          Ellipsis,
    ...
    const channelId = this.ucidOrNone(
      firstString(
        traverseObj<string>(
          renderer,
          [
    ...
    const overlayStyle = firstString(
      traverseObj<string>(
        renderer,
        [
          "thumbnailOverlays",
    ...
    const url =
      overlayStyle === "SHORTS" || navigationUrl.includes("/shorts/")
        ? `https://www.youtube.com/shorts/${videoId}`
        : `https://www.youtube.com/watch?v=${videoId}`;
    const timeText =
      this.getText(renderer, "publishedTimeText", "videoInfo") ?? "";
    const liveStatus =
      overlayStyle === "LIVE" || this.hasBadge(badges, BadgeType.LIVE_NOW)
        ? "is_live"
        : timeText.toLowerCase().includes("streamed")
          ? "was_live"
          : null;
    const viewCountText =
      this.getText(
    ...
    const viewCount = viewCountText.toLowerCase().includes("no views")
      ? 0
      : this.getCount({ simpleText: viewCountText });
    ...
    return this.urlResult(url, YoutubeIE, videoId, title, {
      ...
      channel_is_verified: this.hasBadge(ownerBadges, BadgeType.VERIFIED)
        ? true
        : null,
    });
  }

  protected extractChannelRenderer(
    renderer: Record<string, unknown>,
  ): ExtractorInfo | null {
    ...
    const channelHandle =
      this.extractHandleFromEndpoint(renderer.navigationEndpoint) ??
      this.handleOrNone(this.getText(renderer, "subscriberCountText"));
    return this.urlResult(
      ...
      title,
      {
        ...
        uploader_url: channelHandle
          ? `https://www.youtube.com/${channelHandle}`
          : null,
        channel_follower_count: this.getCount(
          renderer,
          "subscriberCountText",
          "videoCountText",
        ),
        thumbnails: this.extractThumbnails(renderer, "thumbnail"),
        playlist_count:
          this.getCount(renderer, "subscriberCountText") == null
            ? null
            : this.getCount(renderer, "videoCountText"),
        description: this.getText(renderer, "descriptionSnippet"),
        channel_is_verified: this.hasBadge(
          this.extractBadges(renderer.ownerBadges),
          BadgeType.VERIFIED,
        )
          ? true
          : null,
    ...
    for (const item of Array.isArray(items) ? items : []) {
      ...
      if (typeof renderer.playlistId === "string") {
        yield this.urlResult(
          `https://www.youtube.com/playlist?list=${renderer.playlistId}`,
          YoutubeTabIE,
          renderer.playlistId,
          title,
        );
      ...
      for (const Extractor of [
        ...
        if (Extractor.suitable(endpointUrl)) {
          yield this.urlResult(
            endpointUrl,
            Extractor,
            Extractor.getTempId(endpointUrl),
            title,
          );
          break;
        }
  ...
  protected musicResponsiveListEntry(
    renderer: Record<string, unknown>,
  ): ExtractorInfo | null {
    const videoId = nestedString(renderer, ["playlistItemData", "videoId"]);
    if (videoId) {
      const title = nestedString(renderer, [
        "flexColumns",
        0,
    ...
    const playlistId = nestedString(renderer, [
      "navigationEndpoint",
      "watchEndpoint",
      "playlistId",
    ]);
    if (playlistId) {
      const endpointVideoId = nestedString(renderer, [
        "navigationEndpoint",
        "watchEndpoint",
        "videoId",
      ]);
      return endpointVideoId
        ? this.urlResult(
            `https://music.youtube.com/watch?v=${endpointVideoId}&list=${playlistId}`,
            YoutubeTabIE,
            playlistId,
          )
        : this.urlResult(
    ...
    const browseId = nestedString(renderer, [
      "navigationEndpoint",
      "browseEndpoint",
      "browseId",
    ]);
  ...
  protected *shelfEntriesFromContent(
    shelfRenderer: Record<string, unknown>,
  ): Iterable<ExtractorInfo> {
    const content = isRecord(shelfRenderer.content)
  ...
  protected *shelfEntries(
    shelfRenderer: Record<string, unknown>,
    skipChannels = false,
  ): Iterable<ExtractorInfo> {
    const endpointUrl = nestedString(shelfRenderer, [
      "endpoint",
      "commandMetadata",
      "webCommandMetadata",
      "url",
    ]);
    const shelfUrl = endpointUrl
      ? urljoin("https://www.youtube.com", endpointUrl)
      : null;
    if (shelfUrl) {
      ...
      yield this.urlResult(
        ...
        null,
        null,
        this.getText(shelfRenderer, "title"),
      );
    }
    yield* this.shelfEntriesFromContent(shelfRenderer);
  }

  protected *playlistEntries(
    videoListRenderer: Record<string, unknown>,
  ): Iterable<ExtractorInfo> {
    const contents = Array.isArray(videoListRenderer.contents)
      ? videoListRenderer.contents
      : [];
  ...
  protected extractLockupViewModel(
    ...
    const contentType =
      typeof viewModel.contentType === "string" ? viewModel.contentType : null;
    let url: string;
    ...
    if (contentType === "LOCKUP_CONTENT_TYPE_VIDEO") {
      ...
      this.reportWarning(
        `Unsupported lockup view model content type "${contentType}"`,
        null,
        true,
      );
      return null;
  ...
  protected *richEntries(
    richItemRenderer: Record<string, unknown>,
  ): Iterable<ExtractorInfo> {
    const lockupViewModel = isRecord(
      nestedUnknown(richItemRenderer, ["content", "lockupViewModel"]),
    )
      ? (nestedUnknown(richItemRenderer, [
          "content",
          "lockupViewModel",
    ...
    if (typeof renderer.playlistId === "string") {
      yield this.urlResult(
        ...
        YoutubeTabIE,
        renderer.playlistId,
        this.getText(renderer, "title"),
      );
      return;
    }

    if (typeof renderer.entityId === "string") {
      const videoId = nestedString(renderer, [
        "onTap",
        "innertubeCommand",
        "reelWatchEndpoint",
        "videoId",
      ...
      const accessibilityText =
        typeof renderer.accessibilityText === "string"
          ? renderer.accessibilityText
          : "";
      const title =
        nestedString(renderer, ["overlayMetadata", "primaryText", "content"]) ??
        /^(.+), (?:[\d,.]+(?:[KM]| million)?|No) views? - play Short$/.exec(
          accessibilityText,
        )?.[1] ??
        null;
      yield this.urlResult(
        ...
        title,
        {
          view_count: parseCount(
            nestedString(renderer, [
              "overlayMetadata",
              "secondaryText",
              "content",
            ]),
  ...
  protected videoEntry(
    videoRenderer: Record<string, unknown>,
  ): ExtractorInfo | null {
    return typeof videoRenderer.videoId === "string"
      ? this.extractVideo(videoRenderer)
      : null;
  }

  protected hashtagTileEntry(
    hashtagTileRenderer: Record<string, unknown>,
  ): ExtractorInfo | null {
    const url = urljoin(
      "https://youtube.com",
      nestedString(hashtagTileRenderer, [
        "onTapCommand",
        "commandMetadata",
        "webCommandMetadata",
        "url",
    ...
    return url
      ? this.urlResult(
          ...
          YoutubeTabIE,
          null,
          this.getText(hashtagTileRenderer, "hashtag"),
        )
      : null;
  }

  protected *postThreadEntries(
    postThreadRenderer: Record<string, unknown>,
  ): Iterable<ExtractorInfo> {
    const postRenderer = isRecord(
      nestedUnknown(postThreadRenderer, ["post", "backstagePostRenderer"]),
    )
      ? (nestedUnknown(postThreadRenderer, [
          "post",
    ...
    const videoRenderer = firstRecord([
      nestedUnknown(postRenderer, ["backstageAttachment", "videoRenderer"]),
    ]);
    const videoId =
      typeof videoRenderer?.videoId === "string" ? videoRenderer.videoId : null;
    ...
    const playlistId = nestedString(postRenderer, [
      "backstageAttachment",
      "playlistRenderer",
      "playlistId",
    ]);
    ...
    const runs = Array.isArray(
      nestedUnknown(postRenderer, ["contentText", "runs"]),
    )
      ? (nestedUnknown(postRenderer, ["contentText", "runs"]) as unknown[])
      : [];
    for (const run of runs) {
      const endpointUrl = nestedString(run, [
        "navigationEndpoint",
        "urlEndpoint",
        "url",
      ]);
      ...
      const endpointVideoId =
        YoutubeIE.matchValidUrl(endpointUrl)?.groups?.id ?? null;
  ...
  protected *postThreadContinuationEntries(
    postThreadContinuation: Record<string, unknown>,
  ): Iterable<ExtractorInfo> {
    const contents = Array.isArray(postThreadContinuation.contents)
      ? postThreadContinuation.contents
      : [];
    for (const content of contents) {
      if (!isRecord(content)) {
        continue;
  ...
  protected *extractEntries(
    parentRenderer: Record<string, unknown>,
    continuationList: Array<Record<string, unknown> | null>,
  ): Iterable<ExtractorInfo> {
    continuationList[0] = null;
    const contents = Array.isArray(parentRenderer.contents)
      ? parentRenderer.contents
      : [];
    for (const content of contents) {
      if (!isRecord(content)) {
        continue;
      }
      const itemSection = firstRecord([
        content.itemSectionRenderer,
        content.musicShelfRenderer,
        content.musicShelfContinuation,
      ]);
      if (!itemSection) {
        if (isRecord(content.richItemRenderer)) {
          yield* this.richEntries(content.richItemRenderer);
          continuationList[0] =
            YoutubeBaseInfoExtractor.extractContinuation(parentRenderer);
        } else if (isRecord(content.reportHistorySectionRenderer)) {
          yield* this.reportHistoryEntries(
            nestedUnknown(content, [
      ...
      const itemContents = Array.isArray(itemSection.contents)
      ...
      continuationList[0] ??=
        YoutubeBaseInfoExtractor.extractContinuation(itemSection);
    }
    continuationList[0] ??=
      YoutubeBaseInfoExtractor.extractContinuation(parentRenderer);
  }

  private *entriesFromKnownRenderer(
    ...
    for (const [key, value] of Object.entries(item)) {
      ...
      if (key === "playlistVideoListRenderer") {
        ...
        key === "gridPlaylistRenderer" ||
        key === "gridVideoRenderer" ||
        key === "gridChannelRenderer"
      ) {
        entries = this.gridEntries({ items: [{ [key]: value }] });
      } else if (key === "playlistVideoRenderer") {
        entries = this.playlistEntries({
          contents: [{ playlistVideoRenderer: value }],
        });
  ...
  private extractHandleFromBylineRuns(
    renderer: Record<string, unknown>,
  ): string | null {
    const runs = traverseObj<unknown>(renderer, [
      "shortBylineText",
      "runs",
      Ellipsis,
    ]);
    for (const run of Array.isArray(runs) ? runs : []) {
      const handle = this.extractHandleFromEndpoint(
        isRecord(run) ? run.navigationEndpoint : null,
      );
      if (handle) {
        return handle;
      }
  ...
  protected _extract_basic_item_renderer(
    item: unknown,
  ): Record<string, unknown> | null {
    return this.extractBasicItemRenderer(item);
  }

  protected _extract_video(
    renderer: Record<string, unknown>,
  ): ExtractorInfo | null {
    return this.extractVideo(renderer);
  }

  protected _extract_channel_renderer(
    renderer: Record<string, unknown>,
  ): ExtractorInfo | null {
  ...
  protected _music_reponsive_list_entry(
    renderer: Record<string, unknown>,
  ): ExtractorInfo | null {
    return this.musicResponsiveListEntry(renderer);
  }

  protected _shelf_entries_from_content(
    shelfRenderer: Record<string, unknown>,
  ): Iterable<ExtractorInfo> {
    return this.shelfEntriesFromContent(shelfRenderer);
  }

  protected _shelf_entries(
    shelfRenderer: Record<string, unknown>,
    skipChannels = false,
  ): Iterable<ExtractorInfo> {
  ...
  static extractTabRenderers(response: unknown): Record<string, unknown>[] {
    const tabs = nestedUnknown(response, [
      "contents",
    ...
    return (Array.isArray(tabs) ? tabs : []).flatMap((tab) => {
      const renderer = firstRecord([
        nestedUnknown(tab, ["tabRenderer"]),
        nestedUnknown(tab, ["expandableTabRenderer"]),
      ]);
  ...
  protected extractMetadataFromTabs(
    itemId: string,
    ...
    const channelId =
      this.ucidOrNone(nestedString(metadataRenderer, ["externalId"])) ??
      this.ucidFromUrl(nestedString(metadataRenderer, ["channelUrl"]));
    const title =
      nestedString(metadataRenderer, ["title"]) ??
      this.getText(data, ["header", "hashtagHeaderRenderer", "hashtag"]) ??
      itemId;
    const handle =
      this.handleFromUrl(
        nestedString(metadataRenderer, ["vanityChannelUrl"]),
      ) ??
      firstStringDeep(metadataRenderer, [["ownerUrls", Ellipsis]])?.replace(
        /^https?:\/\/(?:www\.)?youtube\.com\//,
        "",
      ) ??
      null;
  ...
  protected _extract_metadata_from_tabs(
    itemId: string,
    data: Record<string, unknown>,
  ): Record<string, unknown> {
  ...
  protected async extractFromTabsWithContinuations(
    itemId: string,
    ...
    const title = [metadata.title, tabTitle, expandedText]
      ...
      .join(" - ");
    return this.playlistResult(
      await this.entriesFromTabWithContinuations(
        selectedTab,
        String(metadata.id ?? itemId),
        ytcfg,
        data,
  ...
  protected async entriesFromTabWithContinuations(
    ...
    for (let pageNum = 1; continuation; pageNum += 1) {
      ...
      const continuationItems = tabContinuationContentItems(
        parsedResponse.data,
      );
      ...
      continuation =
        continuationList[0] ??
        YoutubeBaseInfoExtractor.extractContinuation({
          contents: continuationItems,
        });
    }
    return entries;
...
export class YoutubeTabIE extends YoutubeTabBaseInfoExtractor {
  static override readonly _VALID_URL =
    String.raw`https?://(?!consent\.)(?:\w+\.)?youtube(?:kids)?\.com/(?:(?<channel_type>channel|c|user|browse)/|(?<not_channel>feed/|hashtag/|(?:playlist|watch)\?.*?\blist=)|(?!(?:${YoutubeBaseInfoExtractor._RESERVED_NAMES})\b))(?<id>[^/?#&]+)`;

  static override suitable(url: string): boolean {
    return YoutubeIE.suitable(url)
      ? false
      : YoutubeTabBaseInfoExtractor.suitable.call(this, url);
  }
  ...
  protected override async realExtract(
    ...
    const itemId =
      YoutubeTabIE.matchValidUrl(url)?.groups?.id ??
      YoutubeTabIE.getTempId(url) ??
      url;
    const normalizedUrl = normalizeYoutubeUrl(url);
    const webpage = await this.downloadWebpage(normalizedUrl, itemId, {
    ...
    if (tabs.length) {
      return await this.extractFromTabsWithContinuations(
        itemId,
        ytcfg,
        data,
        tabs,
    ...
    const inlinePlaylist = firstRecord([
      nestedUnknown(data, [
        "contents",
    ...
    if (inlinePlaylist) {
      ...
      return this.playlistResult(
        [...this.playlistEntries(inlinePlaylist)],
        playlistId,
        nestedString(inlinePlaylist, ["title"]),
      );
    }
    ...
    const videoId =
      ...
      parseQs(normalizedUrl).v?.[0] ??
      null;
    if (videoId) {
      this.reportWarning(
        `Unable to recognize playlist. Downloading just video ${videoId}`,
        itemId,
        true,
      );
      return this.urlResult(
...
export class YoutubePlaylistIE extends YoutubeTabIE {
  static override readonly _VALID_URL =
    String.raw`^(?:(?:https?://)?(?:\w+\.)?youtube(?:kids)?\.com/.*?\?.*?\blist=)?(?<id>${YoutubeBaseInfoExtractor._PLAYLIST_ID_RE})`;
  ...
  protected override async realExtract(
    ...
    const playlistUrl = updateUrlQuery(
      "https://www.youtube.com/playlist",
      Object.keys(qs).length ? qs : { list: playlistId },
    );
...
function tabContinuationContentItems(
  response: Record<string, unknown>,
): unknown[] {
  return [
    nestedUnknown(response, [
    ...
    .map((item) =>
      isDirectContinuationRenderer(item)
        ? { itemSectionRenderer: { contents: [item] } }
        : item,
    );
}

function isDirectContinuationRenderer(
  item: unknown,
  ...
  return (
    ...
    !Object.keys(item).some(
      (key) =>
        ...
        key === "musicShelfContinuation" ||
        key === "richItemRenderer" ||
        key === "reportHistorySectionRenderer",
    )
  );
}
...
function nestedUnknown(
  value: unknown,
  path: readonly (string | number | typeof Ellipsis)[],
): unknown {
  const read = (
    current: unknown,
    keys: readonly (string | number | typeof Ellipsis)[],
  ): unknown[] => {
    if (!keys.length) {
      return [current];
    }
    ...
    if (key === Ellipsis) {
      return (Array.isArray(current) ? current : []).flatMap((item) =>
        read(item, rest),
      );
    }
  ...
  const values = read(value, path).filter(
...
function nestedString(
  ...
  if (Array.isArray(result)) {
    return (
      result.find((item): item is string => typeof item === "string") ?? null
    );
  }
  return null;
}
...
function firstStringDeep(
  value: unknown,
  paths: readonly (readonly (string | number | typeof Ellipsis)[])[],
): string | null {
