...
abstract class YoutubeSearchBaseIE extends YoutubeTabBaseInfoExtractor {
  protected async searchResults(
    ...
    while (nextQuery && entries.length < maxResults) {
      ...
      const pageEntries = [
        ...this._extract_entries(
          { contents: searchContentItems(parsedResponse.data) },
          continuationList,
        ),
      ];
      ...
      if (
        !continuation ||
        (continuationToken && seenContinuations.has(continuationToken))
      ) {
        break;
      }
...
export class YoutubeSearchIE extends YoutubeSearchBaseIE {
  ...
  static override readonly _VALID_URL =
    String.raw`ytsearch(?<prefix>|[1-9][0-9]*|all):(?<id>[\s\S]+)`;
  ...
  protected override async realExtract(
    ...
    return this.playlistResult(
      await this.searchResults(
        query,
        YoutubeSearchIE._SEARCH_PARAMS,
        "web",
        maxResults,
      ),
      query,
...
export class YoutubeSearchURLIE extends YoutubeSearchBaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?youtube\.com/(?:results|search)\?([^#]+&)?(?:search_query|q)=(?:[^&]+)(?:[&#]|$)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    return this.playlistResult(
      await this.searchResults(query, qs.sp?.[0], "web"),
      query,
      query,
    );
  }
...
export class YoutubeMusicSearchURLIE extends YoutubeSearchBaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://music\.youtube\.com/search\?([^#]+&)?(?:search_query|q)=(?:[^&]+)(?:[&#]|$)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    if (params) {
      section =
        Object.entries(YoutubeMusicSearchURLIE.SECTIONS).find(
          ([, value]) => value === params,
        )?.[0] ?? params;
    } else {
      section =
      ...
      params = section
        ? (YoutubeMusicSearchURLIE.SECTIONS[section] ?? null)
        : null;
      if (!params) {
        section = null;
      }
    ...
    return this.playlistResult(
      await this.searchResults(query, params, "web_music"),
      title,
      title,
    );
  }
