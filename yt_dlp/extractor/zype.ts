...
export class ZypeIE extends InfoExtractor {
  ...
  private static readonly COMMON_RE =
    String.raw`//player\.zype\.com/embed/${ZypeIE.ID_RE}\.(?:js|json|html)\?.*?(?:access_token|(?:ap[ip]|player)_key)=`;
  static override readonly _VALID_URL =
    String.raw`https?://player\.zype\.com/embed/(?<id>${ZypeIE.ID_RE})\.(?:js|json|html)\?.*?(?:access_token|(?:ap[ip]|player)_key)=[^&]+`;
  static override readonly _EMBED_REGEX = [
    String.raw`<script[^>]+\bsrc=(["'])(?<url>(?:https?:)?${ZypeIE.COMMON_RE}.+?)\1`,
  ];

  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    const videoId = this.matchId(url);
    const jsonUrl = url.replace(/\.(?:js|html)\?/, ".json?");
    const responseJson = await this.downloadJson<{
      response?: { body?: unknown; video?: ZypeVideo };
    }>(jsonUrl, videoId);
    ...
    if (body && typeof body === "object" && !Array.isArray(body)) {
      ...
      for (const output of bodyRecord.outputs ?? []) {
        ...
        if (output.name === "m3u8") {
          [formats, subtitles] = await this.extractM3u8FormatsAndSubtitles(
            output.url,
            videoId,
            "mp4",
          ...
          formats.push({
            ...
            ...(output.name === "m4a" || output.name === "mp3"
              ? { vcodec: "none" }
              : {
                  height: intOrNone(output.height) ?? undefined,
                  width: intOrNone(output.width) ?? undefined,
                }),
          });
        }
      }
      ...
      if (!m3u8Url) {
        const source = this.searchRegex(
          /sources\s*:\s*\[\s*({[\s\S]+?})\s*\]/,
          body,
          "source",
        ) as string;
        const integration = this.searchRegex(
          /\bintegration\s*:\s*(['"])(?<val>(?:(?!\1).)+)\1/,
          source,
          "integration",
          { group: "val" },
        ) as string;
        const id = this.searchRegex(
          /\bid\s*:\s*(['"])(?<val>(?:(?!\1).)+)\1/,
          source,
          "id",
          { group: "val" },
      ...
      const textTracksSource = this.searchRegex(
        /textTracks\s*:\s*(\[[^\]]+\])/,
        body,
        "text tracks",
        { defaultValue: null },
      ...
      if (textTracksSource) {
        textTracks = this.parseJson(textTracksSource, videoId, {
    ...
    if (Array.isArray(textTracks)) {
      for (const track of textTracks) {
        ...
        const trackUrl = dictGet(record as Record<string, string>, [
          "file",
          "src",
        ]);
        if (!trackUrl) {
        ...
        const label =
          typeof record.label === "string" ? record.label : "English";
        subtitles[label] ??= [];
    ...
    return {
      ...
      title: video.title,
      thumbnails: (video.thumbnails ?? []).flatMap((thumbnail) =>
        thumbnail.url
          ? [
              {
                url: thumbnail.url,
      ...
      description:
        video.description ?? video.ott_description ?? video.short_description,
