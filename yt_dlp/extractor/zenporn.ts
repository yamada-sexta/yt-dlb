...
import { determineExt, ExtractorError, unifiedStrdate, urlOrNone } from "../utils/index.ts";
...
export class ZenPornIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://(?:www\.)?zenporn\.com/video/(?<id>\d+)`;

  private genInfoUrl(extDomain: string, extrId: string, lifetime = 86400): string {
    const id = Number(extrId);
    const path = [1_000_000, 1_000, 1].map((divisor) => Math.trunc(id / divisor) * divisor).join("/");
  ...
  private static decodeVideoUrl(encodedUrl: string): string | null {
    ...
    const normalized = [...encodedUrl].map((char) => map[char] ?? char).join("");
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    if (webpage === false) {
      throw new ExtractorError("Unable to download ZenPorn webpage", { videoId: displayId });
    }
    ...
    if (!extDomain || !videoId) {
      throw new ExtractorError("Unable to extract embed info", { videoId: displayId });
    }

    const infoJson = await this.downloadJson<ZenPornInfo>(this.genInfoUrl(extDomain, videoId), videoId, { fatal: false });
    ...
    if (videoJson === false || !videoJson[0]?.video_url) {
      throw new ExtractorError("Unable to extract video file metadata", { videoId });
    }
    ...
    return {
      ...
      ext: traverseObj(videoJson, [0, "format", new Set([determineExt])], { get_all: false }) as string | undefined,
