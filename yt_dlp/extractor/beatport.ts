...
export class BeatportIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://(?:www\.|pro\.)?beatport\.com/track/(?<display_id>[^/]+)/(?<id>[0-9]+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const playablesJson = this.searchRegex(/window\.Playables\s*=\s*({[\s\S]+?});/, webpage, "playables info");
    ...
    const track = playables?.tracks?.find((item) => item.id === Number(trackId));
    if (!track) {
      throw new Error(`Unable to find Beatport track ${trackId}`);
    }

    const artistText = (track.artists ?? []).map((artist) => artist.name).filter(Boolean).join(", ");
    const title = `${artistText}${artistText ? " - " : ""}${track.name ?? trackId}${track.mix ? ` (${track.mix})` : ""}`;
    const formats = Object.entries(track.preview ?? {}).flatMap(([ext, info]) => {
      if (!info.url) {
        return [];
      }
      return [{
    ...
    const thumbnails = Object.entries(track.images ?? {}).flatMap(([name, info]) => {
