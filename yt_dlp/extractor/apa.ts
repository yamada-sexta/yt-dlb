...
export class APAIE extends InfoExtractor {
  static override readonly _VALID_URL =
    String.raw`(?<base_url>https?://[^/]+\.apa\.at)/embed/(?<id>[\da-f]{8}-[\da-f]{4}-[\da-f]{4}-[\da-f]{4}-[\da-f]{12})`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const jwplatformId = this.searchRegex(
      /media[iI]d\s*:\s*["'](?<id>[a-zA-Z0-9]{8})/,
      webpage,
      "jwplatform id",
      {
        defaultValue: null,
    ...
    const extract = (field: string, name = field): string | null => {
      const result = this.searchRegex(
        new RegExp(
          `\\b${RegExp.escape(field)}["']\\s*:\\s*(["'])(?<value>(?:(?!\\1).)+)\\1`,
        ),
        webpage,
        name,
        {
    ...
    for (const formatId of ["hls", "progressive"]) {
      ...
      if (determineExt(sourceUrl) === "m3u8") {
        ...
        const height = this.searchRegex(/(\d+)\.mp4/, sourceUrl, "height", {
          defaultValue: null,
        });
        formats.push({
          url: sourceUrl,
