...
function extractMoment(
  ...
  const uploader = tryGet(
    item,
    (value) => (value as { owner?: { name?: string } }).owner?.name,
    (value): value is string => typeof value === "string",
  );
  const uploaderId = tryGet(
    item,
    (value) => (value as { owner?: { userId?: unknown } }).owner?.userId,
  );
  const title =
  ...
  return {
    ...
    uploader_url:
      formatField(uploader, null, "https://www.younow.com/%s") ?? undefined,
    formats: [
      {
...
export class YouNowLiveIE extends InfoExtractor {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?younow\.com/(?<id>[^/?#&]+)`;

  static override suitable(url: string): boolean {
    return YouNowChannelIE.suitable(url) || YouNowMomentIE.suitable(url)
      ? false
      : super.suitable(url);
  }
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    if (!data) {
      throw new ExtractorError("Unable to download YouNow broadcast info", {
        videoId: username,
      });
    }
    if (data.errorCode !== 0) {
      throw new ExtractorError(
        String(data.errorMsg ?? "YouNow broadcast is unavailable"),
        { expected: true, videoId: username },
      );
    }
    const uploader =
      tryGet(
        data,
        (value) =>
          (value as { user?: { profileUrlString?: string } }).user
            ?.profileUrlString,
        (value): value is string => typeof value === "string",
      ) ?? username;
    return {
...
export class YouNowChannelIE extends InfoExtractor {
  static override readonly _VALID_URL =
    String.raw`https?://(?:www\.)?younow\.com/(?<id>[^/]+)/channel`;

  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    const username = this.matchId(url);
    const info = (await this.downloadJson<Record<string, unknown>>(
      `https://api.younow.com/php/api/broadcast/info/curId=0/user=${username}`,
      username,
      { note: "Downloading user information" },
    )) as Record<string, unknown> | false;
    const channelId = String(info ? info.userId : username);
    const page = (await this.downloadJson<Record<string, unknown>>(
      `${CDN_API_BASE}/moment/profile/channelId=${channelId}/createdBefore=0/records=20`,
      username,
      { note: "Downloading moments page 1" },
    )) as Record<string, unknown> | false;
...
export class YouNowMomentIE extends InfoExtractor {
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const item = (await this.downloadJson<{ item?: Record<string, unknown> }>(
      `${MOMENT_URL_FORMAT}${videoId}`,
      videoId,
    )) as { item?: Record<string, unknown> } | false;
    if (!item || !item.item) {
