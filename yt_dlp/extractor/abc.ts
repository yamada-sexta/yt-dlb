...
export class ABCIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://(?:www\.)?abc\.net\.au/(?:news|btn|listen)/(?:[^/?#]+/){1,4}(?<id>\d{5,})`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    let mobj = /<a\s+href="(?<url>[^"]+)"\s+data-duration="\d+"\s+title="Download audio directly">/.exec(webpage);
    ...
    if (mobj) {
      ...
      mobj = /<a href="(?<url>http:\/\/www\.youtube\.com\/watch\?v=[^"]+)"><span><strong>External Link:<\/strong>/.exec(webpage);
      if (!mobj) {
        mobj = /<iframe width="100%" src="(?<url>\/\/www\.youtube-nocookie\.com\/embed\/[^?"]+)/.exec(webpage);
      }
      if (mobj) {
        urlsInfo = { url: mobj.groups?.url };
        youtube = true;
    ...
    if (!mobj) {
      mobj = /(?:"sources"|"files"|"renditions"):\s*(?<json_data>\[[^\]]+\])/.exec(webpage);
      if (!mobj) {
        mobj = /inline(?<type>Video|Audio|YouTube)Data\.push\((?<json_data>[^)]+)\);/.exec(webpage);
        if (!mobj) {
          const expired = this.htmlSearchRegex(/class="expired-(?:video|audio)".+?<span>(.+?)<\/span>/, webpage, "expired", { defaultValue: null });
          if (expired) {
            throw new ExtractorError(`${this.constructor.name} said: ${expired}`, { expected: true });
          }
          throw new ExtractorError("Unable to extract video urls");
        }
      }

      urlsInfo = this.parseJson<any>(
        mobj.groups?.json_data || "",
        videoId,
        { transform_source: jsToJson }
      );
      youtube = mobj.groups?.type === "YouTube";
      video = mobj.groups?.type === "Video" || traverseObj(
        urlsInfo,
        [0, ["contentType", "MIMEType"]],
        { getAll: false }
      ) === "video/mp4";
    }
    ...
    for (const urlInfo of urlsInfoList) {
      ...
      const matchUrl = /_(?:(?<height>\d+)|(?<bitrate>\d+)k)\.mp4$/.exec(urlInfo.url);
      if (matchUrl) {
...
export class ABCIViewIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://iview\.abc\.net\.au/(?:[^/]+/)*video/(?<id>[^/?#]+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const title = unescapeHTML(videoParams.title || videoParams.seriesTitle) || videoId;
    const stream = videoParams.playlist.find((s: any) => ["program", "livestream"].includes(s?.type));
    if (!stream) {
    ...
    const sig = crypto.createHmac("sha256", Buffer.from("android.content.res.Resources"))
      .update(path)
      .digest("hex");
    ...
    return {
      ...
      season_number: intOrNone(this.searchRegex(/\bSeries\s+(\d+)\b/, title, "season number", { fatal: false, defaultValue: null })) ?? undefined,
      episode_number: intOrNone(this.searchRegex(/\bEp\s+(\d+)\b/, title, "episode number", { fatal: false, defaultValue: null })) ?? undefined,
      episode_id: houseNumber,
      episode: this.searchRegex(/^(?:Series\s+\d+)?\s*(?:Ep\s+\d+)?\s*(.*)$/, title, "episode", { fatal: false, defaultValue: "" }) || undefined,
...
export class ABCIViewShowSeriesIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://iview\.abc\.net\.au/show/(?<id>[^/]+)(?:/series/\d+)?$`;
  static override readonly _GEO_COUNTRIES = ["AU"];
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    if (highlight && !this.yesPlaylist(showId, true, { videoLabel: "highlight video" })) {
      return this.urlResult(String(highlight), "ABCIView");
    }

    const series = videoData.selectedSeries || {};
    const entries: ExtractorInfo[] = [];
    const items = series._embedded?.videoEpisodes?.items || series._embedded?.videoEpisodes || [];
    ...
    const thumbnail = traverseObj(series, [
      "thumbnail",
      "images",
      (v: any) => v.name === "seriesThumbnail",
      "url",
    ], { getAll: false }) as string | null;
