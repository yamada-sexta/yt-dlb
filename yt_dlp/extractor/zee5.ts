...
export class Zee5IE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`(?:zee5:|https?://(?:www\.)?zee5\.com/(?:[^#?]+/)?(?:(?:tv-shows|kids|web-series|zee5originals)(?:/[^#/?]+){3}|(?:movies|kids|videos|news|music-videos)/(?!kids-shows)[^#/?]+)/(?<display_id>[^#/?]+)/)(?<id>[^#/?]+)/?(?:$|[?#])`;
  ...
  protected async performLogin(username: string, password: string | null): Promise<void> {
    const loginHint = 'Use "--username <mobile_number>" to login using otp or "--username token" and "--password <user_token>" to login using user token.';
    if (/^\d{10}$/.test(username) && this.userToken === null) {
      const otpRequest = await this.downloadJson<{ code?: number; message?: string }>(
        `https://b2bapi.zee5.com/device/sendotp_v1.php?phoneno=91${username}`,
        username,
        { note: "Sending OTP" },
      );
      if (otpRequest === false || otpRequest.code !== 0) {
        throw new ExtractorError(otpRequest === false ? "Unable to request OTP" : otpRequest.message ?? "Unable to request OTP", { expected: true });
      }
      const otpCode = this.getTfaInfo("OTP");
      if (!otpCode) {
        throw new ExtractorError("Zee5 OTP login requires the twofactor parameter", { expected: true });
      }
      const otpVerify = await this.downloadJson<{ token?: string; message?: string }>(
      ...
      if (!this.userToken) {
        throw new ExtractorError(otpVerify ? otpVerify.message ?? "Unable to verify OTP" : "Unable to verify OTP", { expected: true });
      }
    ...
    if (intOrNone(token.exp) !== null && intOrNone(token.exp)! <= Math.trunc(Date.now() / 1000)) {
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const launch = await this.downloadJson<{ platform_token?: { token?: string } }>(
    ...
    if (!platformToken) {
      throw new ExtractorError("Unable to extract Zee5 platform token", { videoId });
    }
    const requestData: Record<string, string> = { "x-access-token": platformToken };
    if (this.userToken) {
    ...
    const jsonData = await this.downloadJson<Zee5Playback>(
      ...
      videoId,
      {
        query: {
          ...
          country: this.userCountry ?? this.getParam<string | null>("geo_bypass_country", null) ?? "IN",
    ...
    if (jsonData === false) {
      throw new ExtractorError("Unable to download Zee5 playback data", { videoId });
    }
    const asset = jsonData.assetDetails;
    const businessType = Array.isArray(asset.business_type) ? asset.business_type : [asset.business_type ?? ""];
    if (businessType.includes("premium") || businessType.some((item) => String(item).includes("premium"))) {
      throw new ExtractorError("Premium content is DRM protected.", { expected: true, videoId });
    }
    if (!asset.hls_url) {
      this.raiseLoginRequired('Use "--username <mobile_number>" to login using otp or "--username token" and "--password <user_token>" to login using user token.', { metadataAvailable: true, method: null });
    }
    const [formats, m3u8Subs] = await this.extractM3u8FormatsAndSubtitles(asset.hls_url, videoId, "mp4", { fatal: false });

    const subtitles: Record<string, unknown[]> = {};
    for (const sub of asset.subtitle_url ?? []) {
      if (sub.url) {
        subtitles[sub.language ?? "en"] ??= [];
        subtitles[sub.language ?? "en"]!.push({ url: this.protoRelativeUrl(sub.url) ?? sub.url });
      }
    }
    this.mergeSubtitles(m3u8Subs, subtitles);

    const showData = jsonData.showDetails ?? {};
    const seasons = tryGet(showData, (value) => (value as { seasons?: unknown }).seasons);
    const firstSeason = Array.isArray(seasons) ? seasons[0] as Record<string, unknown> | undefined : undefined;
    return {
      id: videoId,
      ...
      season: strOrNone((showData.seasons as Record<string, unknown> | undefined)?.title ?? firstSeason?.title) ?? undefined,
...
export class Zee5SeriesIE extends InfoExtractor {
  ...
  static override readonly _VALID_URL = String.raw`(?:zee5:series:|https?://(?:www\.)?zee5\.com/(?:[^#?]+/)?(?:tv-shows|web-series|kids|zee5originals)/(?!kids-movies)(?:[^#/?]+/){2})(?<id>[^#/?]+)(?:/episodes)?/?(?:$|[?#])`;
  ...
  private async entries(showId: string): Promise<ExtractorInfo[]> {
    ...
    if (!token) {
      throw new ExtractorError("Unable to extract Zee5 platform token", { videoId: showId });
    }
    const headers = { "X-Access-Token": token, Referer: "https://www.zee5.com/" };
    const showJson = await this.downloadJson<{ seasons?: Array<{ id?: string }> }>(
      `https://gwapi.zee5.com/content/tvshow/${showId}?translation=en&country=IN`,
      showId,
      { headers },
    );
    if (showJson === false) {
      throw new ExtractorError("Unable to download Zee5 show metadata", { videoId: showId });
    }
    ...
    for (const season of showJson.seasons ?? []) {
      ...
      while (nextUrl) {
        pageNum += 1;
        const episodesJson = await this.downloadJson<{ episode?: Array<{ id?: string }>; next_episode_api?: string }>(
          nextUrl,
          showId,
          { headers, note: `Downloading JSON metadata page ${pageNum}` },
        );
        ...
        for (const episode of episodesJson.episode ?? []) {
          if (episode.id) {
            entries.push(this.urlResult(`zee5:${episode.id}`, Zee5IE, episode.id));
          }
        }
