...
export class BreitBartIE extends InfoExtractor {
  static override readonly _VALID_URL = String.raw`https?://(?:www\.)?breitbart\.com/videos/v/(?<id>[^/?#]+)`;
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const formats = this.extractM3u8Formats(`https://cdn.jwplayer.com/manifests/${videoId}.m3u8`, videoId, "mp4");
    return {
      id: videoId,
      title: this.ogSearchTitle(webpage) ?? this.htmlExtractTitle(webpage) ?? videoId,
