...
abstract class ZDFBaseIE extends InfoExtractor {
  ...
  protected async getApiToken(): Promise<string> {
    if (!Object.keys(ZDFBaseIE.tokenCache).length) {
      ZDFBaseIE.tokenCache = ((await this.cache?.load(
        "zdf",
        "api-token",
        "json",
        {},
    ...
    if (
      (intOrNone(ZDFBaseIE.tokenCache.expires) ?? 0) <
      ...
      const token = await this.downloadJson<ZdfToken>(
        ...
        "zdf",
        {
          ...
          errnote: "Failed to download API token",
        },
      );
  ...
  protected extractChapters(
    ...
    const chapters = items.flatMap((item) => {
      ...
      return startTime === null
        ? []
        : [{ start_time: startTime, title: stringValue(record.anchorLabel) }];
    });
    return chapters.length ? chapters : null;
  }

  static extractSubtitles(
    src: unknown,
  ...
  protected expandPtmdTemplate(apiBaseUrl: string, template: string): string {
    return (
      urljoin(apiBaseUrl, template.replace("{playerId}", "android_native_6")) ??
      template
    );
  }

  protected async extractPtmd(
    ptmdUrls: string | readonly string[],
    videoId: string,
    apiToken: string | null = null,
    ...
    for (const rawPtmdUrl of variadic(ptmdUrls)) {
      ...
      const ptmd = await this.callApi<Record<string, unknown>>(
        ptmdUrl,
        videoId,
        "PTMD data",
        apiToken,
      ...
      const basename =
        stringValue(ptmd.basename) ??
        /\/vod\/ptmd\/[^/?#]+\/(\w+)/.exec(ptmdUrl)?.[1] ??
        null;
      if (!contentId && !isDgs) {
        contentId = basename;
      }
      duration ??= floatOrNone(
        getPath(ptmd, ["attributes", "duration", "value"]),
        1000,
      );
      captions.push(
        ...asArray(ptmd.captions).filter(
          (item) => item && typeof item === "object",
        ),
      );

      for (const stream of asArray(ptmd.priorityList).flatMap((item) =>
        asArray(getRecord(item).formitaeten),
      )) {
        ...
        for (const quality of asArray(streamRecord.qualities)) {
          ...
          for (const variant of asArray(
            getPath(qualityRecord, ["audio", "tracks"]),
          )) {
            ...
            if (ext === "m3u8") {
              fmts = this.extractM3u8Formats(formatUrl, videoId, "mp4", {
                m3u8Id: "hls",
              });
              ...
              this.reportWarning(
                `Skipping unsupported extension "${ext}"`,
                videoId,
              );
            }
            const fClass = stringValue(variantRecord.class);
            for (const format of fmts) {
              const fLang = ISO639Utils.short2long(
                String(
                  format.language ?? variantRecord.language ?? "",
                ).toLowerCase(),
              );
              ...
              formats.push({
                ...
                format_note: joinNonempty(
                  !isAudioOnly && fClass,
                  isDgs && "German Sign Language",
                  format.format_note,
                  { delim: ", " },
                ),
                ...
                language: fLang,
                language_preference:
                  (isAudioOnly && format.format_note === "Audiodeskription") ||
                  (!isAudioOnly && fClass === "ad")
                    ? -10
                    : fLang === "deu" && fClass === "main"
                      ? 10
                      : fLang === "deu"
                        ? 5
                        : fClass === "main"
  ...
  protected async downloadGraphql<T>(
    itemId: string,
    dataDesc: string,
    ...
    if (data === false) {
      throw new ExtractorError(`Unable to download ${dataDesc}`, {
        videoId: itemId,
      });
    }
    return data as T;
...
query VideoByCanonical($canonical: String!) {
  ...
  protected override async extractPtmd(
    ...args: Parameters<ZDFBaseIE["extractPtmd"]>
  ): Promise<ExtractorInfo> {
    ...
    return {
      ...ptmdData,
      id: undefined,
      _old_archive_ids: oldArchiveId
        ? [makeArchiveId(this, oldArchiveId)]
        : undefined,
    };
  }
  ...
  private async extractFallback(documentId: string): Promise<ExtractorInfo> {
    ...
    if (video === false) {
      throw new ExtractorError("Unable to download fallback metadata", {
        videoId: documentId,
      });
    }
    const document = getRecord(video.document);
    const ptmdUrl =
      urlOrNone(document.streamApiUrlAndroid) ??
      urlOrNone(getPath(document, ["streams", 0, "streamApiUrlAndroid"]));
    if (!ptmdUrl) {
      throw new ExtractorError("Unable to extract PTMD URL", {
        videoId: documentId,
      });
    }
    const thumbnails = Object.entries(getRecord(document.teaserBild)).flatMap(
      ([key, value]) => {
    ...
    return {
      ...
      timestamp:
        unifiedTimestamp(document.date) ??
        unifiedTimestamp(getPath(video, ["meta", "editorialDate"])) ??
        undefined,
      subtitles: ZDFBaseIE.extractSubtitles(document.captions),
      ...(await this.extractPtmd(
        ptmdUrl,
        documentId,
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const graphql = await this.downloadGraphql<{
      data?: { videoByCanonical?: Record<string, unknown> | null };
    ...
    for (const node of asArray(getPath(videoData, ["currentMedia", "nodes"]))) {
      ...
      if (nodeRecord.vodMediaType) {
        ptmdUrl = smuggleUrl(ptmdUrl, {
          vod_media_type: nodeRecord.vodMediaType,
        });
      }
    ...
    return {
      ...
      description:
        stringValue(videoData.leadParagraph) ??
        stringValue(getPath(videoData, ["teaser", "description"])),
      timestamp:
        parseIso8601(stringValue(videoData.editorialDate)) ?? undefined,
      thumbnails: ZDFBaseIE.extractThumbnails(
        getPath(videoData, ["teaser", "image", "list"]),
      ),
      episode_number:
        intOrNone(getPath(videoData, ["episodeInfo", "episodeNumber"])) ??
        undefined,
      season_number:
        intOrNone(getPath(videoData, ["episodeInfo", "seasonNumber"])) ??
        undefined,
      series: stringValue(getPath(videoData, ["smartCollection", "title"])),
      series_id: stringValue(
        getPath(videoData, ["smartCollection", "canonical"]),
      ),
      chapters:
...
export class ZDFChannelIE extends ZDFBaseIE {
  static override readonly _VALID_URL =
    String.raw`https?://www\.zdf\.de/(?:[^/?#]+/)*(?<id>[^/?#]+)`;
  ...
  private async fetchPage(
    ...
    const response = await this.downloadGraphql<{
      data?: { smartCollectionByCanonical?: Record<string, unknown> };
    }>(playlistId, `season ${seasonNumber} page ${pageNumber} JSON`, {
      query: {
  ...
  private async entries(
    ...
    for (const [seasonIdx, seasonNumber] of seasonNumbers.entries()) {
      ...
      for (let pageNumber = 1; ; pageNumber += 1) {
          ...
          for (const episode of asArray(getPath(node, ["episodes", "nodes"]))) {
            ...
            entries.push(
              this.urlResult(
                sharingUrl,
                ZDFIE,
                stringValue(episodeRecord.canonical) ?? undefined,
                stringValue(getPath(episodeRecord, ["teaser", "title"])) ??
                  undefined,
                {
                  description:
                    stringValue(episodeRecord.leadParagraph) ??
                    stringValue(
                      getPath(episodeRecord, ["teaser", "description"]),
                    ),
                  timestamp:
                    parseIso8601(stringValue(episodeRecord.editorialDate)) ??
                    undefined,
                  episode_number:
                    intOrNone(
                      getPath(episodeRecord, ["episodeInfo", "episodeNumber"]),
                    ) ?? undefined,
                  season_number:
                    intOrNone(
                      getPath(episodeRecord, ["episodeInfo", "seasonNumber"]),
                    ) ?? undefined,
                },
              ),
            );
        ...
        const pageInfo = getRecord(
          getPath(nodes.at(-1), ["episodes", "pageInfo"]),
        );
  ...
  protected override async realExtract(url: string): Promise<ExtractorInfo> {
    ...
    const playlistId = joinNonempty(
      canonicalId,
      seasonNumber && `s${seasonNumber}`,
    );
    const collection = await this.downloadGraphql<{
      data?: { smartCollectionByCanonical?: Record<string, unknown> };
    }>(playlistId, "smart collection data", {
      query: {
        ...
        extensions: JSON.stringify({
          persistedQuery: {
            version: 1,
            sha256Hash:
              "cb49420e133bd668ad895a8cea0e65cba6aa11ac1cacb02341ff5cf32a17cd02",
          },
        }),
      },
    });
    ...
    const seasonNumbers = asArray(
      getPath(collectionData, ["seasons", "seasons"]),
    )
    ...
    if (
      !this.yesPlaylist(
        seasonNumbers.length ? playlistId : null,
        videoUrl ? (stringValue(videoData.canonical) ?? true) : null,
      )
    ) {
      return this.urlResult(
        videoUrl!,
        ZDFIE,
        stringValue(videoData.canonical) ?? undefined,
      );
    }
    if (seasonNumber !== null && !seasonNumbers.includes(seasonNumber)) {
      throw new ExtractorError(
        `Season ${seasonNumber} was not found in the collection data`,
        { videoId: playlistId },
      );
    }
    return this.playlistResult(
      ...
      playlistId,
      joinNonempty(
        stringValue(collectionData.title),
