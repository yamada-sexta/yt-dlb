...
export class MhtmlFD extends FragmentFD {
  override async realDownload(filename: string, info: DownloadInfo): Promise<boolean> {
    const rawFragments = Array.isArray(info.fragments) ? info.fragments as FragmentInfo[] : [];
    const fragments = this.params.test ? rawFragments.slice(0, 1) : rawFragments;
    if (!fragments.length) {
    ...
    try {
      ...
      for (const [index, fragment] of fragments.entries()) {
        const fragmentUrl = fragment.url ?? new URL(fragment.path ?? "", String(info.fragment_base_url ?? info.url)).toString();
        const bytes = new Uint8Array(await (await this.ydl.urlopen(fragmentUrl)).arrayBuffer());
        ...
        writer.write(
          ...
          + (typeof fragment.duration === "number" ? `X.ytdlb.Duration: ${fragment.duration}\r\n` : "")
          + "\r\n",
        );
        writer.write(bytes);
        writer.write("\r\n");
  ...
  private generateStub(fragments: readonly FragmentInfo[], boundary: string, title: string): string {
    const figures = fragments.map((fragment, index) => {
      const duration = typeof fragment.duration === "number" ? ` (duration: ${formatDuration(fragment.duration)})` : "";
