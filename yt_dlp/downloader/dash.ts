...
export class DashSegmentsFD extends FragmentFD {
  override async realDownload(
    ...
    const formats = Array.isArray(info.requested_formats)
      ? (info.requested_formats as DownloadInfo[])
      : [info];
    if (info.is_live) {
      if (FFmpegFD.available()) {
        return await new FFmpegFD(this.ydl, this.params).download(
          filename,
          info,
        );
      }
    ...
    for (const [index, format] of formats.entries()) {
      const target =
        formats.length === 1
          ? filename
          : typeof format.filepath === "string"
            ? format.filepath
            : null;
      if (!target) {
        throw new NotImplementedError(
          "DASH multi-format download without per-format filepath",
        );
      }
      result =
        (await this.downloadFormat(
          target,
          { ...info, ...format },
          index === 0,
        )) && result;
  ...
  private async downloadFormat(
    ...
    const base =
      typeof format.fragment_base_url === "string"
        ? format.fragment_base_url
        : format.url;
    const extraQuery =
      typeof format.extra_param_to_segment_url === "string"
        ? new URLSearchParams(format.extra_param_to_segment_url)
        : null;
    const sourceFragments = this.params.test
      ? format.fragments.slice(0, 1)
      : format.fragments;
    const fragments = sourceFragments.map((fragment, index): FragmentInfo => {
      const item = fragment as {
        url?: string;
        path?: string;
      ...
      return {
        ...
        url: extraQuery
          ? updateUrlQuery(rawUrl, queryToRecord(extraQuery))
          : rawUrl,
      };
    });
    this.toScreen("[dashsegments] Total fragments: " + fragments.length);
    const external = getExternalFragmentDownloader(this.params, "dash", {
      ...format,
    ...
    if (external) {
      this.toScreen(
        `[dashsegments] Fragment downloads will be delegated to ${external.name.replace(/FD$/, "").toLowerCase()}`,
      );
