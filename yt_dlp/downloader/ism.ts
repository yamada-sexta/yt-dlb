...
export class IsmFD extends FragmentFD {
  override async realDownload(
    ...
    const fragments = (
      this.params.test ? info.fragments.slice(0, 1) : info.fragments
    ).map((fragment, index): FragmentInfo => {
...
function parseDownloadParams(value: unknown): IsmDownloadParams {
  ...
  if (
    typeof data.fourcc !== "string" ||
    typeof data.duration !== "number" ||
    typeof data.stream_type !== "string"
  ) {
  ...
  return {
    ...
    bits_per_sample:
      typeof data.bits_per_sample === "number"
        ? data.bits_per_sample
        : undefined,
    sampling_rate:
      typeof data.sampling_rate === "number" ? data.sampling_rate : undefined,
    codec_private_data:
      typeof data.codec_private_data === "string"
        ? data.codec_private_data
        : undefined,
    nal_unit_length_field:
      typeof data.nal_unit_length_field === "number"
        ? data.nal_unit_length_field
        : undefined,
  };
}
...
function isStreamType(
  value: string,
): value is IsmDownloadParams["stream_type"] {
...
function writePiffHeader(params: IsmDownloadParams): Uint8Array {
  ...
  const ftyp = box(
    "ftyp",
    concatBytes([ascii("isml"), u32(1), ascii("piff"), ascii("iso2")]),
  );
  ...
  const dinf = box(
    ...
    fullBox(
      ...
      0,
      0,
      concatBytes([u32(1), fullBox("url ", 0, 0x1, new Uint8Array())]),
    ),
  );
  ...
  const mvex = box(
    ...
    concatBytes([
      ...
      fullBox(
        ...
        0,
        0,
        concatBytes([u32(trackId), u32(1), u32(0), u32(0), u32(0)]),
      ),
    ]),
  );
...
function sampleEntry(params: IsmDownloadParams): Uint8Array {
  ...
  if (params.stream_type === "video") {
    const codecPrivateData = hexToBytes(
      requiredString(params.codec_private_data, "codec_private_data"),
    );
  ...
  if (params.stream_type === "text") {
    ...
    return box(
      "stpp",
      concatBytes([base, cString("http://www.w3.org/ns/ttml"), u8(0), u8(0)]),
    );
  }
...
function extractBoxData(
  ...
  while (offset + 8 <= data.byteLength) {
    ...
    if (type === boxSequence[0]) {
      return boxSequence.length === 1
        ? payload
        : extractBoxData(payload, boxSequence.slice(1));
    }
    offset += size;
  }
