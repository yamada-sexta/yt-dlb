...
export class RtspFD extends FileDownloader {
  override async realDownload(
    ...
    if (!command) {
      throw new Error(
        'MMS or RTSP download detected but neither "mplayer", "mpv", nor "ffmpeg" could be run. Please install one',
      );
    }
    ...
    if (exitCode !== 0) {
      throw new Error(
        `${command[0]} exited with code ${exitCode}${stderr ? `: ${stderr.trim()}` : ""}`,
      );
    }
