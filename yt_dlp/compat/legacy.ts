...
export const compat_get_terminal_size = (): { columns: number; rows: number } => ({
  columns: process.stdout.columns ?? 80,
  rows: process.stdout.rows ?? 24,
});
export const compat_getenv = (key: string): string | undefined => process.env[key];
...
export const compat_print = (...items: readonly unknown[]): void => console.log(...items);
...
export const compat_zip = <T extends readonly unknown[]>(...iterables: { [K in keyof T]: Iterable<T[K]> }): T[] => {
