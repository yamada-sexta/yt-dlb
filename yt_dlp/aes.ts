...
function algorithmFor(
  key: Uint8Array | readonly number[],
  mode: "cbc" | "ctr" | "ecb" | "gcm",
): string {
...
export function aesCbcDecryptBytes(
  ...
  const decipher = createDecipheriv(
    algorithmFor(key, "cbc"),
    toBuffer(key),
    toBuffer(iv),
  );
  decipher.setAutoPadding(false);
  return new Uint8Array(
    Buffer.concat([decipher.update(toBuffer(data)), decipher.final()]),
  );
}

export function aesCbcEncryptBytes(
  data: Uint8Array,
  key: Uint8Array,
  iv: Uint8Array,
): Uint8Array {
  ...
  return new Uint8Array(
    Buffer.concat([cipher.update(toBuffer(data)), cipher.final()]),
  );
}
...
export function aesEcbEncrypt(data: Uint8Array, key: Uint8Array): Uint8Array {
  ...
  return new Uint8Array(
    Buffer.concat([
      cipher.update(toBuffer(pkcs7Padding(data))),
      cipher.final(),
    ]),
  );
}
...
export function aesEncrypt(
  ...
  expandedKey: Uint8Array | readonly number[],
): Uint8Array {
...
export function aesDecryptText(
  data: string,
  password: string,
  keySizeBytes: 16 | 24 | 32,
): Uint8Array {
  ...
  const derivedBlock = aesEncrypt(
    keyMaterial.subarray(0, BLOCK_SIZE_BYTES),
    keyExpansionKeyMaterial(keyMaterial),
  );
  const key = new Uint8Array(keySizeBytes);
  for (let offset = 0; offset < key.length; offset += BLOCK_SIZE_BYTES) {
    key.set(
      derivedBlock.subarray(0, Math.min(BLOCK_SIZE_BYTES, key.length - offset)),
      offset,
    );
  }
...
function keyFromExpandedKey(
  expandedKey: Uint8Array | readonly number[],
): Uint8Array | null {
  const keyLength =
    expandedKey.length === 176
      ? 16
      : expandedKey.length === 208
        ? 24
        : expandedKey.length === 240
          ? 32
          : null;
...
function keyExpansionKeyMaterial(key: Uint8Array): Uint8Array {
  return key.length === 16
    ? Uint8Array.from([...key, ...Array(160).fill(0)])
    : key.length === 24
      ? Uint8Array.from([...key, ...Array(184).fill(0)])
      : Uint8Array.from([...key, ...Array(208).fill(0)]);
}
